...
// עדכון פרופיל מלא - עכשיו תומך בכל השדות החדשים
router.put('/', async (req: Request, res: Response) => {
  ...
  const { 
    ...
    zodiac, social_link, location, birth_date, 
    relationship_status, education, job_title, signal_price 
  } = req.body;

  try {
    const updates: any = { 
      full_name, username, bio, avatar_url, cover_url, 
      zodiac, social_link, location, relationship_status, 
      education, job_title, signal_price 
    };
    
    if (birth_date === '' || birth_date === null) updates.birth_date = null;
