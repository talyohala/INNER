...
import { Loader2, ArrowDownLeft, ArrowUpRight, Zap, History, ShieldCheck, SmartphoneNfc, ChevronDown, Send, Tag, Banknote, UserCircle, CheckCircle2 } from 'lucide-react';
...
export const WalletPage: React.FC = () => {
  ...
  useEffect(() => {
    const searchUsers = async () => {
      ...
      try {
        ...
        const filtered = safeData.filter((u) => u.id !== currentUserId);
  ...
  const processNativePayment = async () => {
    ...
    setTimeout(async () => {
      try {
        ...
        toast.success(`רכשת ${selectedPackage.amount} CRD בהצלחה! 💎`, {
          id: tid,
  ...
  const selectRecipient = (user: SearchUser) => {
    ...
    // אם אין לו username, נשים את השם המלא שלו כדי שיראה יפה בתיבה
  ...
  const handleTransfer = async () => {
    ...
    // ביטלתי את החסימה המיותרת של username. כל מה שצריך זה רק ID!
    if (!selectedRecipient?.id) {
      return toast.error('בחר משתמש מהרשימה לפני שליחה', { style: { background: '#111', color: '#ef4444' } });
    }

    if (isNaN(amountNum) || amountNum <= 0) {
      return toast.error('אנא הזן סכום תקין', { style: { background: '#111', color: '#ef4444' } });
    }

    if (amountNum > balance) {
      return toast.error('אין לך מספיק יתרה בארנק', { style: { background: '#111', color: '#ef4444' } });
    }
    ...
    try {
      ...
      toast.success(`ההעברה בוצעה בהצלחה ל-${selectedRecipient.full_name || (selectedRecipient.username ? '@' + selectedRecipient.username : 'המשתמש')}!`, {
        id: tid,
        style: { background: '#111', color: '#2196f3', border: '1px solid rgba(33,150,243,0.2)' }
      });
      ...
      console.error(err);
      triggerFeedback('error');
      toast.error(err?.message || 'ההעברה נכשלה', {
        id: tid,
  ...
  const handleRedeem = async () => {
    ...
    setTimeout(() => {
      ...
      triggerFeedback('success');
      toast.success('בקשת הפדיון נשלחה לאישור!', {
        id: tid,
        style: { background: '#111', color: '#4ade80', border: '1px solid rgba(74,222,128,0.2)' }
      });
      ...
      <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ type: 'spring', stiffness: 100, damping: 20 }}>
        <div className="bg-gradient-to-br from-[#1a1a1a] to-[#050505] border border-white/20 pt-8 pb-4 rounded-[36px] flex flex-col items-center text-center shadow-[0_25px_60px_rgba(0,0,0,0.8)] relative overflow-hidden z-10">
          ...
          <div className="flex flex-col items-center justify-center mt-2 mb-8 z-10 relative">
            <span className="text-[72px] font-black text-white tracking-tighter drop-shadow-[0_0_20px_rgba(255,255,255,0.1)] leading-none">
          ...
          <div className="w-full flex items-center justify-between px-10 pt-4 border-t border-white/5 relative z-10">
            <button
              ...
              className="flex items-center gap-1.5 text-white/60 hover:text-white transition-colors text-[12px] font-black uppercase tracking-widest active:scale-95"
            >
              ...
              {visibleTransactions.map((tx, idx) => (
                <div key={tx.id} className={`flex items-center justify-between p-5 ${idx !== 2 && idx !== transactions.length - 1 ? 'border-b border-white/5' : ''}`}>
                  <div className="flex items-center gap-4 text-right">
                    <div
                      className={`w-12 h-12 rounded-[20px] flex items-center justify-center shrink-0 shadow-inner ${
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-white/40 text-[10px] font-bold mt-1 tracking-widest" dir="ltr">
                  ...
                  <span className="font-black text-[18px] text-white tracking-wide" dir="ltr">
                    <span className={tx.type === 'deposit' || tx.type === 'transfer_in' ? 'text-[#e5e4e2] pl-0.5' : 'text-white/30 pl-0.5'}>
                      {tx.type === 'deposit' || tx.type === 'transfer_in' ? '+' : '-'}
                    </span>
                    {tx.amount}
                  </span>
                ...
                >
                  ...
                  <div className="p-6 flex flex-col items-center text-center gap-6 overflow-y-auto scrollbar-hide">
                    ...
                    <Button
                      ...
                      disabled={adding}
                      className="w-full h-14 mt-2 bg-white text-black font-black text-[14px] uppercase tracking-widest rounded-[20px] flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(255,255,255,0.2)] transition-all active:scale-95 disabled:opacity-50"
                    >
            ...
            {showTransfer && (
              <div className="fixed inset-0 z-[9900] flex flex-col justify-end" dir="rtl">
                ...
                <motion.div
                  ...
                  exit={{ y: '100%' }}
                  transition={{ type: 'spring', damping: 25, stiffness: 200 }}
                  className="bg-[#0A0A0A] border-t border-white/10 rounded-t-[36px] h-[90vh] flex flex-col shadow-[0_-20px_50px_rgba(0,0,0,0.8)] relative pb-12 overflow-hidden"
                >
                  ...
                  <div className="p-6 flex flex-col gap-6 overflow-y-auto scrollbar-hide">
                    <div className="flex flex-col gap-2 relative z-20">
                      ...
                      <div className="relative">
                        <input
                          ...
                          dir="rtl"
                          className="bg-black/40 text-white font-medium h-14 border border-white/10 shadow-inner focus:border-white/50 focus:outline-none transition-all rounded-[20px] px-4 w-full"
                        />
                        ...
                        <AnimatePresence>
                          {showUserDropdown && transferUsername.trim() !== '' && !selectedRecipient && (
                            <motion.div
                              ...
                              className="mt-2 bg-[#111] border border-white/10 rounded-[20px] shadow-inner overflow-hidden flex flex-col"
                            >
                              {isSearchingUsers ? (
                      ...
                      {selectedRecipient && (
                        <div className="mt-2 bg-[#2196f3]/10 border border-[#2196f3]/20 rounded-[18px] p-3 flex items-center justify-between">
                          ...
                          <button
                            ...
                            className="text-white/40 text-[11px] font-black"
                          >
                            שנה
                          </button>
                        </div>
                    ...
                    <div className="flex flex-col gap-2 relative z-0">
                      ...
                      <input
                        ...
                        placeholder="100..."
                        dir="ltr"
                        className="bg-black/40 text-white font-black h-14 border border-white/10 shadow-inner focus:border-white/50 focus:outline-none transition-all rounded-[20px] px-4 text-xl w-full"
                      />
                    </div>
                    ...
                    <Button
                      ...
                      className="w-full h-14 mt-4 bg-white text-black font-black text-[14px] uppercase tracking-widest rounded-[20px] flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(255,255,255,0.2)] transition-all active:scale-95 disabled:opacity-50 relative z-0"
                    >
                  ...
                  <div className="p-6 flex flex-col gap-6 overflow-y-auto scrollbar-hide">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <input
                        ...
                        placeholder="0"
                        dir="ltr"
                        className="bg-black/40 text-white font-black h-14 border border-white/10 shadow-inner focus:border-[#4ade80]/50 focus:outline-none transition-all rounded-[20px] px-4 text-xl w-full"
                      />
                    </div>
                    ...
                    <Button
                      ...
                      disabled={redeeming}
                      className="w-full h-14 mt-4 bg-white text-black font-black text-[14px] uppercase tracking-widest rounded-[20px] flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(255,255,255,0.2)] transition-all active:scale-95 disabled:opacity-50"
                    >
