...
// מונע מהמסך להיתקע אם משהו חושב יותר מדי זמן
const withTimeout = <T,>(promise: Promise<T>, ms: number = 5000): Promise<T> => {
  return Promise.race([
    promise,
    new Promise<T>((_, reject) => setTimeout(() => reject(new Error('זמן קריאה חלף')), ms))
  ]);
};
...
export const HomePage: React.FC = () => {
  ...
  // חילוץ נתונים ישירות מסופאבייס - עוקף את השרת התקוע לחלוטין
  const fetchData = async (isSilentRefresh = false) => {
    ...
    try {
      ...
      let fetchedCircles = [];
      try {
        const { data } = await withTimeout(supabase.from('circles').select('*'));
        if (data) fetchedCircles = data;
      } catch (e) {}

      // משיכת פוסטים כולל פרופילים, לייקים ותגובות
      let fetchedPosts = [];
      try {
        ...
        if (data) {
          fetchedPosts = data.map(p => ({
            ...
            is_liked: p.post_likes?.some((l: any) => l.user_id === currentUserId)
          }));
        }
      } catch (e) {}

      setCircles(fetchedCircles.sort((a: any, b: any) => (b.members_count || 0) - (a.members_count || 0)));
  ...
  const handleLike = async (postId: string, isLiked: boolean) => {
    ...
    try {
      ...
      if (!authData.user) return;
      if (isLiked) {
        await supabase.from('post_likes').delete().match({ post_id: postId, user_id: authData.user.id });
      } else {
        await supabase.from('post_likes').insert({ post_id: postId, user_id: authData.user.id });
      }
  ...
  const openComments = async (post: any) => {
    ...
    try {
      const { data, error } = await supabase.from('post_comments').select(`*, profiles(*)`).eq('post_id', post.id).order('created_at', { ascending: true });
      setComments(data || []);
      ...
      {activePost && (
        <div className="fixed inset-0 z-[99999] flex flex-col justify-end" dir="rtl">
          ...
          <motion.div drag="y" dragControls={dragControls} dragListener={false} dragConstraints={{ top: 0, bottom: 0 }} dragElastic={0.2} onDragEnd={(e, { offset, velocity }) => { if (offset.y > 100 || velocity.y > 400) setActivePost(null); }} initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="bg-[#0A0A0A] border-t border-white/10 rounded-t-[36px] h-[85vh] flex flex-col shadow-[0_-20px_50px_rgba(0,0,0,0.8)] relative overflow-hidden">
            ...
            <div className="p-5 border-t border-white/10 bg-black/90 backdrop-blur-2xl mt-auto pb-8" onPointerDown={e => e.stopPropagation()} onTouchStart={e => e.stopPropagation()}>
  ...
  return (
    <>
      <div className="px-4 pt-8 pb-32 bg-[#030303] min-h-screen relative overflow-x-hidden touch-pan-y" dir="rtl" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
                  ...
                  <div key={post.id} className="p-6 rounded-[36px] bg-white/[0.04] backdrop-blur-2xl border border-white/10 relative overflow-hidden shadow-2xl">
                    ...
                    {post.media_url && (
                      <div className="mb-5 rounded-[24px] overflow-hidden border border-white/10 bg-[#050505] shadow-inner">
                        {post.media_url.match(/\.(mp4|webm|mov)$/i) || post.media_type === 'video' ? (
                          <video src={post.media_url} autoPlay loop muted playsInline className="w-full h-auto max-h-[450px] object-cover" />
                        ) : (
