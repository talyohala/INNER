...
import {
  ...
  UserCircle, ChevronLeft, ChevronDown, ChevronUp, ArrowUp, MoreVertical, 
  Share2, Download, Bookmark, Link, Reply, Trash2, Edit2
} from 'lucide-react';
  ...
  return (
    <>
            ...
            return (
              <div key={post.id} className="flex flex-col bg-[#0A0A0A] border-y border-white/5 overflow-hidden shadow-lg rounded-[32px] w-full">
                ...
                {!hasMedia && post.content && (
                  <div className="p-6 pb-4 pt-6 border-b border-white/5">
                ...
                {/* הפוטר של הפוסט - מועדונים (אם יש), יוצר הפוסט מצד ימין ופעולות מצד שמאל */}
                <div className="flex flex-col p-4 px-5 bg-[#0A0A0A] rounded-b-[32px] gap-4">
                  
                  {post.user_circles && post.user_circles.length > 0 && (
                    <div className="flex flex-col">
                      <span className="text-white/30 text-[10px] font-black uppercase tracking-wider text-right mb-2">שייך למועדונים:</span>
                      <div className="flex gap-4 overflow-x-auto scrollbar-hide items-center justify-end">
                        {post.user_circles.slice(0, 10).map((circle: any) => (
                          <div key={circle.id} onClick={() => navigate(`/circle/${circle.slug || circle.id}`)} className="flex items-center gap-2 shrink-0 cursor-pointer active:scale-95 transition-transform bg-white/5 rounded-full pr-1 pl-3 py-1">
                            ...
                            <span className="text-[10px] text-white/80 font-bold truncate max-w-[60px] text-right">{circle.name}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="flex items-center justify-between w-full">
                    {/* צד שמאל - פעולות (לייק, תגובה, 3 נקודות) */}
                    <div className="flex items-center gap-4" dir="ltr">
                      <button onClick={() => openOverlay(() => setOptionsMenuPost(post))} className="text-white/40 hover:text-white transition-colors p-1"><MoreVertical size={20}/></button>
                      <button onClick={() => openOverlay(() => { setActivePost(post); setActiveCommentsPostId(post.id); setLoadingComments(true); supabase.from('comments').select('*, profiles(*)').eq('post_id', post.id).order('created_at', { ascending: true }).then((r) => { setComments(r.data || []); setLoadingComments(false); }); })} className="flex items-center gap-1.5 text-white/60 hover:text-white transition-all active:scale-90">
                        <span className="text-[14px] font-black">{post.comments_count}</span> <MessageSquare size={20} />
                      </button>
                      <button onClick={() => handleLike(post.id, post.is_liked)} className={`flex items-center gap-1.5 transition-all active:scale-90 ${post.is_liked ? 'text-red-500' : 'text-white/60 hover:text-white'}`}>
      ...
      <AnimatePresence>
              ...
              {fullScreenMedia.map((vid, idx) => {
                ...
                return (
                  <div key={keyVal} className="w-full h-screen snap-center relative bg-black flex items-center justify-center">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 px-6 pb-8 pt-24 bg-gradient-to-t from-black/90 via-black/40 to-transparent flex flex-col justify-end pointer-events-none">
                      
                      {/* מועדונים מעל משתמש (במסך המלא) */}
                      {vid.user_circles && vid.user_circles.length > 0 && (
                        <div className="flex items-center justify-end gap-2 w-full overflow-x-auto scrollbar-hide mb-4 pointer-events-auto">
                      ...
                      <div className="flex items-center justify-between w-full pointer-events-auto pb-2">
                          {/* שמאל - פעולות */}
                          <div className="flex items-center gap-4" dir="ltr">
                          ...
                          {/* ימין - משתמש */}
                          <div className="flex items-center gap-3 cursor-pointer group" onClick={(e) => { e.stopPropagation(); closeOverlay(); setTimeout(() => navigate(`/profile/${vid.user_id}`), 50); }}>
                            <div className="flex flex-col text-right">
                      ...
                      {vid.content && (
                        <p className="text-white/90 text-[15px] font-medium text-right w-5/6 line-clamp-3 pointer-events-auto cursor-pointer mt-2" onClick={(e) => { e.stopPropagation(); openOverlay(() => setActiveDescPost(vid)); }}>{vid.content}</p>
                      )}
                    </div>
                  </div>
                );
        ...
        {commentActionModal && (
           <div className="fixed inset-0 z-[1000000] flex flex-col justify-end" onTouchStart={stopPropagation} onTouchMove={stopPropagation}>
        ...
        {showCreatePost && (
          <div className="fixed inset-0 z-[999999] flex flex-col justify-end" onTouchStart={stopPropagation} onTouchMove={stopPropagation}>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-0 bg-black/80 backdrop-blur-sm" onClick={closeOverlay} />
            <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="relative z-10 bg-[#0A0A0A] rounded-t-[40px] p-6 flex flex-col gap-4 pb-12 shadow-[0_-10px_50px_rgba(0,0,0,0.8)] border-t border-white/10">
              <div className="w-full py-4 flex justify-center"><div className="w-16 h-1.5 bg-white/20 rounded-full" /></div>
              ...
              <textarea value={newPost} onChange={(e) => setNewPost(e.target.value)} placeholder="כתוב משהו..." className="h-32 bg-[#111] rounded-[24px] p-5 text-white text-[16px] outline-none resize-none border border-white/10 placeholder:text-white/40 shadow-inner" />
              {!editingPost && (
