...
import {
  Loader2, Bell, Heart, MessageSquare, Send, X, Paperclip, RefreshCw, 
  UserCircle, ChevronLeft, ChevronDown, ChevronUp, ArrowUp, MoreVertical, Share2
} from 'lucide-react';
...
export const HomePage: React.FC = () => {
  ...
  const handleCopyLink = async (post: any) => {
    const publicUrl = `https://inner-app.com/post/${post.id}`;
    try { await navigator.clipboard.writeText(publicUrl); toast.success('הקישור הועתק ללוח'); } catch { toast.error('שגיאה בהעתקה'); }
    closeOverlay();
  };
  ...
  const handleSavePost = async (post: any) => {
    try {
      ...
      if (e?.code === '23505') toast.success('הפוסט כבר שמור אצלך');
  ...
  const handleContainerScroll = (e: React.UIEvent<HTMLDivElement>) => {
    ...
    scrollTimeout.current = setTimeout(() => {
      ...
      if (target.scrollHeight - target.scrollTop <= target.clientHeight * 2 && fullScreenMedia) {
        ...
        const more = posts.filter((p) => p.media_url && !existingIds.has(p.id)).sort(() => Math.random() - 0.5).slice(0, 3);
  ...
  if (loading && posts.length === 0) return <div className="min-h-screen bg-[#030303] flex items-center justify-center"><Loader2 className="animate-spin text-white/20" /></div>;
    ...
    <>
      ...
      <AnimatePresence>
        {showScrollTop && !isAnyModalOpen() && (
          <motion.button initial={{ opacity: 0, scale: 0.5, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.5, y: 20 }} onClick={scrollToTop} className="fixed bottom-24 right-5 z-[80] w-12 h-12 bg-white/10 backdrop-blur-md border border-white/20 rounded-full flex items-center justify-center shadow-lg active:scale-90">
            <ArrowUp size={24} className="text-white" />
          </motion.button>
        )}
      </AnimatePresence>

      <FadeIn className="pt-8 pb-32 bg-[#030303] min-h-screen relative overflow-x-hidden touch-pan-y" dir="rtl" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
        ...
        <div className="flex flex-col gap-8 w-full px-0 relative z-10">
          {posts.map((post) => {
            ...
            return (
              <div key={post.id} className="flex flex-col bg-[#0A0A0A] border-y border-white/5 overflow-hidden shadow-lg w-full">
                      ...
                      {post.user_circles.slice(0, 10).map((circle: any) => (
                        <div key={circle.id} onClick={() => navigate(`/circle/${circle.slug || circle.id}`)} className="flex flex-col items-center gap-1.5 shrink-0 cursor-pointer active:scale-95 transition-transform">
                          <div className="w-12 h-12 rounded-full border border-white/10 overflow-hidden bg-[#111]">
                            {circle.cover_url ? <img src={circle.cover_url} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center font-black text-white/20">{circle.name?.charAt(0)}</div>}
                          </div>
        ...
        <AnimatePresence>
          ...
          {commentActionModal && (
            <div className="fixed inset-0 z-[99999999] flex flex-col justify-end" onTouchStart={stopPropagation} onTouchMove={stopPropagation}>
              ...
              <motion.div drag="y" dragConstraints={{ top: 0, bottom: 0 }} dragElastic={0.2} onDragEnd={(e, info) => { if (info.offset.y > 100) closeOverlay(); }} initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="relative z-10 bg-[#0A0A0A] rounded-t-[40px] p-6 flex flex-col gap-3 pb-12 shadow-[0_-10px_50px_rgba(0,0,0,0.8)] border-t border-white/10">
            ...
            <div className="fixed inset-0 z-[9999999] flex flex-col justify-end" onTouchStart={stopPropagation} onTouchMove={stopPropagation}>
              ...
              <motion.div drag="y" dragConstraints={{ top: 0, bottom: 0 }} dragElastic={0.2} onDragEnd={(e, info) => { if (info.offset.y > 100) closeOverlay(); }} initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} className="relative z-10 bg-[#0A0A0A] rounded-t-[40px] p-6 flex flex-col gap-2 pb-12 shadow-[0_-10px_50px_rgba(0,0,0,0.8)] border-t border-white/10">
                ...
                {optionsMenuPost.media_url && (
                  <button onClick={() => handleDownloadMedia(optionsMenuPost.media_url)} className="w-full p-4 bg-white/5 rounded-full text-white/90 font-bold flex justify-center items-center text-lg active:bg-white/10 transition-colors">שמור למכשיר</button>
                )}
                <button onClick={() => handleSavePost(optionsMenuPost)} className="w-full p-4 bg-white/5 rounded-full text-white/90 font-bold flex justify-center items-center text-lg active:bg-white/10 transition-colors">שמור במועדפים</button>
                <button onClick={() => handleCopyLink(optionsMenuPost)} className="w-full p-4 bg-white/5 rounded-full text-white/90 font-bold flex justify-center items-center text-lg active:bg-white/10 transition-colors">העתק קישור</button>
                <button onClick={() => { closeOverlay(); setTimeout(() => handleShare(optionsMenuPost), 100); }} className="w-full p-4 bg-[#2196f3]/10 rounded-full text-[#2196f3] font-bold flex justify-center items-center text-lg active:bg-[#2196f3]/20 transition-colors mt-2">שתף פוסט</button>
                {optionsMenuPost.user_id === currentUserId && (
                  <>
                    <button onClick={() => { closeOverlay(); setTimeout(() => { openOverlay(() => { setEditingPost(optionsMenuPost); setNewPost(optionsMenuPost.content || ''); setShowCreatePost(true); }); }, 100); }} className="w-full p-4 bg-white/5 rounded-full text-white/90 font-bold flex justify-center items-center text-lg active:bg-white/10 transition-colors mt-4">ערוך פוסט</button>
                    <button onClick={() => { if (window.confirm('למחוק פוסט?')) { deletePost(optionsMenuPost.id); } }} className="w-full p-4 bg-red-500/10 border border-red-500/20 rounded-full text-red-500 font-bold flex justify-center items-center text-lg mt-2 active:bg-red-500/20 transition-colors">מחק פוסט</button>
                  </>
                )}
              </motion.div>
            </div>
