...
import { Loader2, Bell, Users, Lock, Flame, Heart, MessageSquare, Send, X, Paperclip, RefreshCw, UserCircle, Trash2, Edit2, Reply } from 'lucide-react';
...
export const HomePage: React.FC = () => {
  ...
  const fetchData = async (isSilentRefresh = false) => {
    ...
    try {
      ...
      // התיקון: טעינת מועדונים מסופאבייס ישירות
      const { data: circlesData } = await supabase.from('circles').select('*, circle_members(user_id)');
      const formattedCircles = (circlesData || []).map((c: any) => ({
        ...c,
        is_member: c.circle_members?.some((m: any) => m.user_id === uid)
      })).sort((a: any, b: any) => (b.members_count || 0) - (a.members_count || 0));

      // התיקון הקריטי: קריאה לטבלאות likes ו- comments הנכונות
      const { data: postsData } = await supabase
        .from('posts')
        .select('*, profiles(id, username, full_name, avatar_url), likes(user_id), comments(id)')
        .is('circle_id', null)
  ...
  const submitComment = async () => {
    ...
    try {
      const { data, error } = await supabase.from('comments').insert({
        post_id: activePost.id, user_id: currentUserId, content: newComment.trim()
  ...
  const goToProfile = (userId: string | undefined) => {
    if (!userId) { toast.error('פרטי המשתמש חסרים'); return; }
