    ...
    scrollTimeout.current = setTimeout(() => {
      ...
      if (target.scrollHeight - target.scrollTop <= target.clientHeight * 2.5) {
        if (fullScreenVideos) {
          ...
          const more = [...vids].sort(() => Math.random() - 0.5).slice(0, 3);
    ...
    <>
      <FadeIn className="px-0 pt-8 pb-32 bg-[#030303] min-h-screen relative overflow-x-hidden touch-pan-y" dir="rtl" onTouchStart={e => { if (window.scrollY <= 0) pullStartY.current = e.touches[0].clientY; }} onTouchMove={e => { if (pullStartY.current > 0 && window.scrollY <= 0) { const y = e.touches[0].clientY - pullStartY.current; if (y > 0) setPullY(Math.min(y, 120)); } }} onTouchEnd={() => { if (pullY > 60) { setRefreshing(true); setPullY(0); fetchData(true); } else setPullY(0); pullStartY.current = 0; }}>
        ...
        <div className="relative z-10 px-4">
          ...
          <div className="p-5 rounded-[36px] mb-8 border border-white/10 bg-white/[0.04] backdrop-blur-3xl shadow-2xl relative z-10 transition-colors cursor-pointer" onClick={() => openOverlay(() => setShowCreatePost(true))}>
            ...
            <div className="flex justify-end items-center gap-4 border-t border-white/10 pt-4 mt-1 px-1">
              ...
              <div className="w-12 h-12 p-0 rounded-full bg-white text-black flex items-center justify-center shrink-0 shadow-[0_0_15px_rgba(255,255,255,0.2)]"><Send size={20} className="rtl:-scale-x-100 -ml-1 text-[#2196f3]" /></div>
            </div>
          </div>
        </div>
              ...
              <div key={post.id} className="flex flex-col rounded-[36px] bg-[#0A0A0A] border border-white/10 relative overflow-hidden shadow-[0_10px_40px_rgba(0,0,0,0.5)]">
                  ...
                  <div className={`w-full bg-[#050505] relative ${isVideo ? 'cursor-pointer' : ''}`} onClick={() => isVideo ? openOverlay(() => { const vids = posts.filter(p => p.media_type === 'video' || (p.media_url && p.media_url.match(/\.(mp4|webm|mov)$/i))); const others = vids.filter(p => p.id !== post.id).sort(()=>Math.random()-0.5); setFullScreenVideos([post, ...others]); }) : null}>
                    ...
                    {post.content && (
                      <div className="absolute bottom-0 left-0 right-0 p-5 pt-16 bg-gradient-to-t from-[#0A0A0A] via-black/60 to-transparent flex items-end pointer-events-none">
                        <p onClick={(e) => { e.stopPropagation(); openOverlay(() => setActiveDescPost(post)); }} className="text-white/90 text-[14px] leading-relaxed font-medium text-right line-clamp-2 cursor-pointer active:opacity-50 transition-opacity pointer-events-auto drop-shadow-lg w-full pr-2">{post.content}</p>
                      </div>
                    )}
                  </div>
                )}
                ...
                {!post.media_url && post.content && (
                  <div className="p-6 pb-4 border-b border-white/5">
                    <p onClick={() => openOverlay(() => setActiveDescPost(post))} className="text-white/90 text-[15px] leading-relaxed font-medium text-right line-clamp-3 cursor-pointer active:opacity-50 transition-opacity">{post.content}</p>
                  </div>
                )}
                ...
                <div className={`flex items-center justify-between px-5 py-4 bg-[#0A0A0A] border-t border-white/10`}>
                  <div className="flex items-center justify-start gap-3 cursor-pointer group" onClick={() => navigate(`/profile/${post.user_id}`)}>
                    <div className="w-10 h-10 rounded-full bg-black border border-white/10 overflow-hidden shrink-0 shadow-inner group-hover:opacity-80 transition-opacity">
                      {post.profiles?.avatar_url ? <img src={post.profiles.avatar_url} className="w-full h-full object-cover" /> : <UserCircle size={18} className="text-white/20 w-full h-full p-2" />}
                    </div>
                    <div className="flex flex-col text-right">
                      <span className="text-white font-black text-[14px]">{post.profiles?.full_name || 'אנונימי'}</span>
                      <span className="text-white/30 text-[10px] font-bold mt-0.5">{new Date(post.created_at).toLocaleDateString('he-IL')}</span>
                    </div>
                  </div>

                  <div className="flex items-center gap-5 flex-row-reverse">
                    {isMyPost && <button onClick={() => openOverlay(() => setOptionsMenuPost(post))} className="flex items-center justify-center text-white/30 hover:text-white active:scale-90"><MoreVertical size={18} /></button>}
                    <button onClick={() => openOverlay(() => { setActivePost(post); setActiveCommentsPostId(post.id); setLoadingComments(true); supabase.from('comments').select('*, profiles(*)').eq('post_id', post.id).order('created_at',{ascending:true}).then(r => setComments(r.data||[])); })} className="flex items-center gap-1.5 text-white/30 hover:text-[#2196f3] active:scale-90"><MessageSquare size={18} /><span className="text-[12px] font-black">{post.comments_count}</span></button>
                  ...
                  <div key={vid.id + '-' + idx} className="w-full h-screen snap-center relative bg-black flex items-center justify-center">
                    ...
                    <div className="absolute bottom-32 left-4 flex flex-col gap-6 items-center z-50 pointer-events-auto">
                      <button onClick={(e) => { e.stopPropagation(); handleLike(vid.id, vid.is_liked); }} className="flex flex-col items-center gap-1.5 active:scale-90 transition-transform">
                        ...
                        <span className="text-white font-black text-[13px] drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">{vid.likes_count}</span>
                      </button>
                      <button onClick={(e) => { e.stopPropagation(); openOverlay(() => { setActivePost(vid); setActiveCommentsPostId(vid.id); setLoadingComments(true); supabase.from('comments').select('*, profiles(*)').eq('post_id', vid.id).order('created_at',{ascending:true}).then(r => setComments(r.data||[])); }); }} className="flex flex-col items-center gap-1.5 active:scale-90 transition-transform">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-6 pb-12 bg-gradient-to-t from-black/90 via-black/40 to-transparent pointer-events-none flex flex-col gap-3">
                       <div className="flex items-center justify-start gap-3 w-full pr-2">
                         <div className="w-12 h-12 rounded-full overflow-hidden bg-black border-2 border-white/20 shrink-0 shadow-lg pointer-events-auto cursor-pointer" onClick={() => { closeOverlay(); navigate(`/profile/${vid.user_id}`); }}>
                         ...
                         <div className="flex flex-col text-right">
                           <span className="text-white font-black text-[17px] drop-shadow-md pointer-events-auto cursor-pointer" onClick={() => { closeOverlay(); navigate(`/profile/${vid.user_id}`); }}>{vid.profiles?.full_name || 'אנונימי'}</span>
                         </div>
                       </div>
        ...
        <AnimatePresence>
          {showCreatePost && (
            <div className="fixed inset-0 z-[999999] flex flex-col justify-end" dir="rtl">
              ...
              <motion.div drag="y" dragControls={createPostDragControls} dragListener={false} dragConstraints={{ top: 0, bottom: 0 }} onDragEnd={(e, { offset }) => { if (offset.y > 100) closeOverlay(); }} initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="bg-[#0A0A0A] border-t border-white/10 rounded-t-[36px] flex flex-col shadow-[0_-20px_50px_rgba(0,0,0,0.8)] relative overflow-hidden pb-10 px-6 pt-6">
                <div className="w-full flex justify-center pb-6 cursor-grab active:cursor-grabbing bg-white/[0.02]" onPointerDown={(e) => createPostDragControls.start(e)} style={{ touchAction: "none" }}><div className="w-16 h-1.5 bg-white/20 rounded-full"></div></div>
                ...
                <div className="flex flex-col gap-4">
                  <input type="text" onChange={e => setSelectedFile(null)} placeholder="העלאת תמונות ווידאו כרגע רק מהכפתור מהדף הראשי..." disabled className="h-14 bg-white/5 border border-white/10 rounded-[20px] px-4 text-white/30 text-sm focus:outline-none" dir="rtl" />
                  ...
                  <Button onClick={handlePost} disabled={posting || (!newPost.trim() && !editingPost)} className="h-14 bg-[#2196f3] text-white font-black rounded-[20px]">{posting ? <Loader2 className="animate-spin" /> : 'פרסם'}</Button>
                </div>
              </motion.div>
            </div>
          )}
              ...
              <motion.div drag="y" dragControls={optionsDragControls} dragListener={false} dragConstraints={{ top: 0, bottom: 0 }} onDragEnd={(e, { offset }) => { if (offset.y > 50) closeOverlay(); }} initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="bg-[#0A0A0A] border-t border-white/10 rounded-t-[36px] flex flex-col shadow-[0_-20px_50px_rgba(0,0,0,0.8)] relative overflow-hidden pb-10">
                ...
                <div className="flex flex-col p-4 gap-2">
                  ...
                  <button onClick={() => { closeOverlay(); setTimeout(() => openOverlay(() => { setEditingPost(optionsMenuPost); setNewPost(optionsMenuPost.content || ''); setShowCreatePost(true); }), 100); }} className="flex items-center justify-between w-full p-4 rounded-2xl bg-white/5 hover:bg-white/10 transition-colors text-white font-black text-lg border border-white/5">
                    <span>ערוך פוסט</span><Edit2 size={20} className="text-white/50" />
                  </button>
                  <button onClick={() => { deletePost(optionsMenuPost.id); closeOverlay(); }} className="flex items-center justify-between w-full p-4 rounded-2xl bg-red-500/10 hover:bg-red-500/20 transition-colors text-red-500 font-black text-lg border border-red-500/20 mt-2">
          ...
          {activeDescPost && (
            <div className="fixed inset-0 z-[999999] flex flex-col justify-end" dir="rtl">
              ...
              <motion.div drag="y" dragControls={descDragControls} dragListener={false} dragConstraints={{ top: 0, bottom: 0 }} onDragEnd={(e, { offset }) => { if (offset.y > 100) closeOverlay(); }} initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="bg-[#0A0A0A] border-t border-white/10 rounded-t-[36px] max-h-[75vh] flex flex-col shadow-[0_-20px_50px_rgba(0,0,0,0.8)] relative overflow-hidden pb-10">
                <div className="w-full flex justify-center pt-5 pb-3 cursor-grab active:cursor-grabbing bg-white/[0.02]" onPointerDown={(e) => descDragControls.start(e)} style={{ touchAction: "none" }}><div className="w-16 h-1.5 bg-white/20 rounded-full"></div></div>
                ...
                <div className="flex-1 overflow-y-auto p-6 scrollbar-hide touch-pan-y" onPointerDown={(e) => { if (e.currentTarget.scrollTop > 0) e.stopPropagation(); }}>
                  <p className="text-white/90 text-[15px] leading-relaxed font-medium whitespace-pre-wrap text-right">{activeDescPost.content}</p>
                </div>
              </motion.div>
            </div>
          )}
          ...
          {activeCommentsPostId && (
            <div className="fixed inset-0 z-[999999] flex flex-col justify-end" dir="rtl">
              ...
              <motion.div drag="y" dragControls={commentsDragControls} dragListener={false} dragConstraints={{ top: 0, bottom: 0 }} onDragEnd={(e, { offset }) => { if (offset.y > 100) closeOverlay(); }} initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="bg-[#0A0A0A] border-t border-white/10 rounded-t-[36px] h-[85vh] flex flex-col shadow-[0_-20px_50px_rgba(0,0,0,0.8)] relative overflow-hidden">
                <div className="w-full flex justify-center pt-5 pb-3 cursor-grab active:cursor-grabbing bg-white/[0.02]" onPointerDown={(e) => commentsDragControls.start(e)} style={{ touchAction: "none" }}><div className="w-16 h-1.5 bg-white/20 rounded-full"></div></div>
                    ...
                    comments.map(comment => {
                      ...
                      return (
                        <div key={comment.id} className="flex gap-4">
                          <div className="w-10 h-10 rounded-[16px] bg-black shrink-0 overflow-hidden border border-white/10 shadow-inner p-0.5 cursor-pointer" onClick={() => { closeOverlay(); navigate(`/profile/${targetId}`); }}>
                          ...
                          <div className="flex flex-col flex-1 bg-white/[0.04] p-4 rounded-[24px] rounded-tr-sm border border-white/5 shadow-sm">
                            <span className="text-white font-black text-[13px] mb-1.5 text-right w-fit cursor-pointer hover:text-[#e5e4e2] transition-colors" onClick={() => { closeOverlay(); navigate(`/profile/${targetId}`); }}>{comment.profiles?.full_name || 'אנונימי'}</span>
