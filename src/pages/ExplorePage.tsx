...
import { Search, ChevronLeft, Loader2, UserCircle, X, Lock, Unlock, Gift, TrendingUp, Flame, Crown, Hash } from 'lucide-react';
...
const CATEGORY_TREE = [
  { id: 'all', name: 'הכל' },
  { id: 'venture_capital', name: 'הון סיכון', sub: ['קריפטו', 'נדל"ן', 'סטארטאפים', 'השקעות אלטרנטיביות'] },
  { id: 'business', name: 'עסקים ויזמות', sub: ['איקומרס', 'שיווק', 'נטוורקינג', 'ניהול'] },
  { id: 'tech_alpha', name: 'טק וחדשנות', sub: ['בינה מלאכותית', 'פיתוח', 'סייבר', 'SaaS'] },
  { id: 'lifestyle', name: 'לייף סטייל', sub: ['חיי לילה', 'אופנה', 'טיולים', 'קולינריה'] },
  { id: 'wellness', name: 'גוף ונפש', sub: ['כושר', 'מיינדפולנס', 'תזונה', 'התפתחות אישית'] },
  { id: 'creators', name: 'יוצרים ומדיה', sub: ['פודקאסטים', 'אמנות', 'מוזיקה', 'וידאו'] },
  { id: 'gaming', name: 'גיימינג', sub: ['Esports', 'סטרימינג', 'פיתוח משחקים'] },
  { id: 'sanctum', name: 'The Sanctum', sub: ['לחברים בלבד', 'אירועי VIP', 'הזמנות מיוחדות'] }
];
    ...
    <FadeIn className="pt-[calc(env(safe-area-inset-top)+12px)] pb-32 bg-surface min-h-screen font-sans flex flex-col overflow-x-hidden" dir="rtl">
          ...
          <AnimatePresence>
            {search && (
              <motion.button
                ...
                className="absolute left-4 top-1/2 -translate-y-1/2 text-brand-muted hover:text-brand transition-colors p-1 active:scale-90"
              >
                <X size={18} />
              </motion.button>
            )}
      ...
      <div className="flex-1 mt-4">
        <AnimatePresence mode="wait">
          ...
          {search.trim() ? (
            <motion.div key="search-results" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex flex-col gap-6">
              {loadingSearch ? (
            ...
            <motion.div key="explore-home" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex flex-col gap-8 pb-10">
              ...
              {activeCampaign && (
                <div onClick={() => { 
                  triggerFeedback('pop'); 
                  if (activeCampaign.link) window.open(activeCampaign.link, '_blank'); 
                  else toast.success('קמפיין נפתח!'); 
                }} className="mx-4 relative overflow-hidden rounded-[24px] cursor-pointer group active:scale-[0.98] transition-transform shadow-lg border border-surface-border">
                  ...
                  {activeCampaign.cover_url && (
                    <img src={activeCampaign.cover_url} className="absolute inset-0 w-full h-full object-cover opacity-30 mix-blend-overlay" />
                  )}
                  <div className="relative p-6 flex items-center justify-between z-10">
                    <div className="flex flex-col gap-1.5">
                      <span className="text-slate-300 text-[10px] font-black tracking-widest uppercase flex items-center gap-1.5">
                        <Gift size={12} /> {activeCampaign.sponsor_name || 'דרופ מיוחד'}
                      </span>
                      <span className="text-brand font-black text-[18px] tracking-wide">{activeCampaign.title}</span>
                      <span className="text-brand-muted text-[12px] font-medium leading-relaxed max-w-[180px] line-clamp-2">
                        {activeCampaign.description || 'צפה בקמפיין החדש שלנו'}
                      </span>
                    </div>
                    <button className="h-12 w-12 shrink-0 rounded-full bg-white text-black flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="flex gap-3 overflow-x-auto scrollbar-hide px-4 pb-2 snap-x snap-mandatory">
                    {trendingPosts.map((post) => (
                      <div key={post.id} onClick={() => { triggerFeedback('pop'); navigate(`/post/${post.id}`); }} className="w-[240px] snap-center shrink-0 bg-[#0a0a0a] border border-surface-border rounded-[24px] p-4 flex flex-col gap-3 cursor-pointer shadow-sm active:scale-95 transition-transform group hover:border-slate-400/30">
                        <div className="flex items-center gap-2">
                          <div className="w-8 h-8 rounded-full overflow-hidden bg-surface border border-surface-border shrink-0">
                            {post.profiles?.avatar_url ? <img src={post.profiles.avatar_url} className="w-full h-full object-cover" /> : <UserCircle className="w-full h-full text-brand-muted" />}
                          </div>
                          <div className="flex flex-col">
                            <span className="text-brand font-black text-[12px] flex items-center gap-1 truncate max-w-[150px]">{post.profiles?.full_name || 'אנונימי'} {post.profiles?.role_label === 'CORE' && <Crown size={10} className="text-slate-400"/>}</span>
                          </div>
                        </div>
                        {post.media_url && (
                ...
                <div className="flex flex-col px-4 gap-3">
                  ...
                  <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-2 snap-x">
                    {recentSearches.map((term, i) => (
                      <div key={i} onClick={() => { triggerFeedback('pop'); setSearch(term); }} className="snap-start shrink-0 flex items-center gap-2 bg-surface-card border border-surface-border rounded-full px-4 py-2 cursor-pointer active:scale-95 transition-transform shadow-sm whitespace-nowrap hover:bg-white/5">
              ...
              {/* CATEGORIES - FIXED SCROLLING */}
              <div className="flex flex-col gap-2">
                <div className="flex overflow-x-auto scrollbar-hide px-4 gap-2 pb-2 snap-x">
                  {CATEGORY_TREE.map((cat) => (
                    <button
                      key={cat.id}
                      onClick={() => { triggerFeedback('pop'); setActiveCategory(cat.id); setActiveSub(''); }}
                      className={`snap-start shrink-0 px-5 py-2.5 rounded-full text-[13px] font-black tracking-wide transition-all border active:scale-95 ${
                ...
                <AnimatePresence mode="wait">
                  {currentCatObj && currentCatObj.sub && currentCatObj.sub.length > 0 && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="flex overflow-x-auto scrollbar-hide gap-2 px-4 mt-2 pb-2 snap-x">
                      {currentCatObj.sub.map((subName) => (
                        <button
                          key={subName}
                          onClick={() => { triggerFeedback('pop'); setActiveSub(activeSub === subName ? '' : subName); }}
                          className={`snap-start shrink-0 h-8 px-4 rounded-full font-bold text-[11px] transition-all border active:scale-95 ${
                            activeSub === subName
                    ...
                    {suggestedUsers.map((u, i) => (
                      ...
                      >
                        <div className="flex items-center gap-4 text-right">
                          ...
                          <div className="flex flex-col">
                            <span className="text-brand font-black text-[15px] flex items-center gap-1 truncate max-w-[160px]">
                              {u.full_name || 'משתמש'}
                              {u.role_label === 'CORE' && <Crown size={12} className="text-slate-400" />}
                            </span>
                            <span className="text-brand-muted text-[12px] font-medium mt-0.5" dir="ltr">@{u.username || 'user'}</span>
                        ...
                        <div className="w-8 h-8 rounded-full bg-surface border border-surface-border flex items-center justify-center shadow-inner shrink-0">
                          <ChevronLeft size={16} className="text-brand-muted rtl:rotate-180" />
                        </div>
                      </div>
                    ))}
                  </div>
