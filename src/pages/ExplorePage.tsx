...
import { Search, ChevronLeft, Loader2, UserCircle, X, Lock, Unlock, Gift, TrendingUp, Timer, Flame, Crown } from 'lucide-react';
      ...
      <div className="relative z-20 px-4 sticky top-0 bg-surface/90 backdrop-blur-2xl py-3 shadow-sm border-b border-surface-border">
        <div className="relative">
          <input
            ...
            className="w-full bg-surface-card border border-surface-border rounded-full h-[52px] pr-12 pl-12 text-brand text-[15px] font-bold placeholder:text-brand-muted/50 outline-none transition-all focus:border-indigo-400/50 shadow-inner"
          />
          ...
          {search.trim() ? (
                        ...
                        <div className="flex items-center gap-4 text-right">
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className="text-brand-muted text-[12px] font-medium mt-0.5 flex items-center gap-1.5">
                              ...
                              {circle.join_price > 0 && <span className="text-indigo-400/70 font-black text-[10px] tracking-wider uppercase">• {circle.join_price} CRD</span>}
                            </span>
                          </div>
                        </div>
            ...
            <motion.div key="explore-home" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col gap-8 pb-10">
              <div className="mx-4 relative overflow-hidden rounded-[24px] cursor-pointer group active:scale-[0.98] transition-transform shadow-lg border border-surface-border">
                ...
                <div className="relative p-6 flex items-center justify-between z-10">
                  <div className="flex flex-col gap-1.5">
                    <span className="text-indigo-400 text-[10px] font-black tracking-widest uppercase flex items-center gap-1.5">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="flex gap-3 overflow-x-auto scrollbar-hide px-4 pb-2">
                    {trendingPosts.map((post) => (
                      <div key={post.id} onClick={() => navigate(`/post/${post.id}`)} className="w-[240px] shrink-0 bg-[#0a0a0a] border border-surface-border rounded-[24px] p-4 flex flex-col gap-3 cursor-pointer shadow-sm active:scale-95 transition-transform group hover:border-indigo-400/30">
                        <div className="flex items-center gap-2">
                          ...
                          <div className="flex flex-col">
                            <span className="text-brand font-black text-[12px] flex items-center gap-1">{post.profiles?.full_name || 'אנונימי'} {post.profiles?.role_label === 'CORE' && <Crown size={10} className="text-indigo-400"/>}</span>
                          </div>
                        </div>
                        {post.media_url && (
              ...
              {recentSearches.length > 0 && (
                <div className="flex flex-col px-4 gap-3">
                  <div className="flex items-center justify-between px-1">
                    ...
                    <button onClick={() => { triggerFeedback('pop'); setRecentSearches([]); localStorage.removeItem('inner_recent_searches'); }} className="text-indigo-400 text-[11px] font-black uppercase tracking-widest">נקה</button>
                  </div>
              ...
              <div className="flex flex-col gap-4">
                ...
                {loadingCircles ? (
                  <div className="py-16 flex justify-center"><Loader2 className="animate-spin text-indigo-400" /></div>
                ) : filteredCircles.length === 0 ? (
                  ...
                  <div className="grid grid-cols-2 gap-3 px-4">
                    {filteredCircles.map((circle, idx) => {
                      ...
                      return (
                        ...
                        >
                          <div className="h-[220px] rounded-[24px] bg-surface-card border border-surface-border shadow-sm flex flex-col relative overflow-hidden group hover:border-indigo-400/30">
                        ...
                        <div className="flex items-center gap-4 text-right">
                          ...
                          <div className="flex flex-col">
                            <span className="text-brand font-black text-[15px] flex items-center gap-1">
                              {u.full_name || 'משתמש'}
                              {u.role_label === 'CORE' && <Crown size={12} className="text-indigo-400" />}
                            </span>
                            <span className="text-brand-muted text-[12px] font-medium mt-0.5" dir="ltr">@{u.username || 'user'}</span>
                          </div>
                        </div>
