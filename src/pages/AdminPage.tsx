...
export const AdminPage: React.FC = () => {
  ...
  if (loading) return <div className="min-h-screen bg-[#030303] flex items-center justify-center"><Loader2 className="animate-spin text-indigo-400" size={32} /></div>;
    ...
    <FadeIn className="bg-[#030303] min-h-[100dvh] font-sans flex flex-col relative overflow-hidden pb-24" dir="rtl">
      ...
      <div className="pt-[calc(env(safe-area-inset-top)+20px)] px-6 pb-6 flex justify-between items-end relative z-10">
        <div className="flex flex-col">
          <span className="text-indigo-400 text-[11px] font-black tracking-[0.2em] uppercase mb-1 drop-shadow-md">מערכת הליבה</span>
          <h1 className="text-3xl font-black text-brand tracking-widest flex items-center gap-2 drop-shadow-lg">INNER <ShieldCheck size={28} className="text-indigo-400" /></h1>
        </div>
      </div>
      ...
      <div className="px-4 mb-6 relative z-10 overflow-x-auto scrollbar-hide">
        <div className="flex items-center gap-2 min-w-max pb-1">
          <button onClick={() => { triggerFeedback('pop'); setActiveTab('dashboard'); }} className={`px-5 py-2.5 rounded-full text-[13px] font-medium transition-all ${activeTab === 'dashboard' ? 'bg-surface-card border border-indigo-400/30 text-indigo-300' : 'bg-surface-card border border-surface-border text-brand-muted hover:text-brand'}`}>סקירה</button>
          <button onClick={() => { triggerFeedback('pop'); setActiveTab('cashouts'); }} className={`px-5 py-2.5 rounded-full text-[13px] font-medium transition-all relative ${activeTab === 'cashouts' ? 'bg-surface-card border border-indigo-400/30 text-indigo-300' : 'bg-surface-card border border-surface-border text-brand-muted hover:text-brand'}`}>
          ...
          <button onClick={() => { triggerFeedback('pop'); setActiveTab('users'); }} className={`px-5 py-2.5 rounded-full text-[13px] font-medium transition-all ${activeTab === 'users' ? 'bg-surface-card border border-indigo-400/30 text-indigo-300' : 'bg-surface-card border border-surface-border text-brand-muted hover:text-brand'}`}>משתמשים</button>
          <button onClick={() => { triggerFeedback('pop'); setActiveTab('content'); }} className={`px-5 py-2.5 rounded-full text-[13px] font-medium transition-all ${activeTab === 'content' ? 'bg-surface-card border border-indigo-400/30 text-indigo-300' : 'bg-surface-card border border-surface-border text-brand-muted hover:text-brand'}`}>תוכן</button>
        </div>
      </div>
          ...
          {activeTab === 'dashboard' && (
            <motion.div key="dash" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex flex-col gap-6">
              ...
              <div className="bg-surface-card border border-surface-border pt-10 pb-8 rounded-[40px] flex flex-col items-center text-center shadow-lg relative overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/5 via-transparent to-transparent opacity-60" />
                ...
                <div className="flex flex-col items-center justify-center mt-2 z-10 relative">                                 
                  ...
                  <span className="text-[11px] font-black text-indigo-400 uppercase tracking-[0.4em] mt-3 drop-shadow-md">TOTAL CIRCULATION</span>                                                
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-surface-card border border-surface-border rounded-[28px] p-5 shadow-sm flex flex-col items-center text-center">
                  <span className="text-brand-muted text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 mb-2"><Users size={14} className="text-indigo-400"/> משתמשים</span>
                  <span className="text-brand font-black text-2xl">{adminData.total_users.toLocaleString()}</span>
                </div>
                <div className="bg-surface-card border border-surface-border rounded-[28px] p-5 shadow-sm flex flex-col items-center text-center">
                  <span className="text-brand-muted text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 mb-2"><MessageSquare size={14} className="text-indigo-400"/> פוסטים באוויר</span>
              ...
              <div className="bg-surface-card border border-surface-border rounded-[32px] p-6 shadow-sm">
                <span className="text-brand-muted text-[11px] font-black uppercase tracking-widest flex items-center gap-1.5 mb-6"><Activity size={14} className="text-indigo-400"/> הרשמות השבוע</span>
                      ...
                      return (
                        <div key={i} className="flex flex-col items-center gap-2 flex-1">
                          <div className="w-full bg-surface rounded-t-md flex flex-col justify-end h-full relative group">
                            <motion.div initial={{ height: 0 }} animate={{ height: `${heightPercent}%` }} className="bg-indigo-400/80 rounded-t-md w-full transition-opacity group-hover:bg-indigo-400" />
                ...
                adminData.pending_cashouts.map((tx: any) => (
                  <div key={tx.id} className="bg-surface-card border border-surface-border rounded-[28px] p-5 shadow-sm flex flex-col">
                    <div className="flex items-center justify-between border-b border-surface-border pb-4 mb-4">
                      ...
                      <span className="text-indigo-400 font-black text-xl" dir="ltr">{Math.abs(tx.amount)} CRD</span>
                    </div>
          ...
          {activeTab === 'users' && (
            <motion.div key="users" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex flex-col gap-4">
              <div className="relative">
                ...
                <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="חיפוש משתמשים..." className="w-full bg-surface-card border border-surface-border text-brand font-medium h-14 rounded-full pl-4 pr-12 outline-none focus:border-indigo-400/50 transition-colors shadow-inner text-[14px]" />
              </div>
          ...
          {selectedUser && (
            <div className="flex flex-col gap-6 text-center items-center">
              ...
              <div className="flex flex-col items-center">
                <h2 className="text-brand font-black text-2xl flex items-center gap-2">{selectedUser.full_name} {selectedUser.role_label === 'CORE' && <ShieldCheck size={18} className="text-indigo-400" />}</h2>
              ...
              <div className="w-full bg-surface-card border border-surface-border rounded-[24px] p-5 flex flex-col items-center gap-1 shadow-sm">
                ...
                <span className="text-indigo-400 font-black text-3xl flex items-center gap-2">{selectedUser.crd_balance} <DollarSign size={20} /></span>
              </div>
              ...
              <div className="w-full flex flex-col gap-3 mt-2">
                ...
                <div className="flex items-center gap-2 bg-surface-card border border-surface-border p-2 pr-4 rounded-2xl">
                  <Gift size={20} className="text-indigo-400 shrink-0" />
                  ...
                  <Button onClick={handleGrantCrd} disabled={actionLoading || !grantAmount} className="h-10 bg-indigo-500/20 text-indigo-300 rounded-xl px-4 text-[12px] font-black shadow-sm active:scale-95 disabled:opacity-50">שלח בונוס</Button>
                </div>
