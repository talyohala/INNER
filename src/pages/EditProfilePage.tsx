...
const GENDER_OPTIONS = [
  { name: 'זכר', icon: <span className="text-blue-400 text-[18px] leading-none font-black">♂</span> },
  { name: 'נקבה', icon: <span className="text-pink-400 text-[18px] leading-none font-black">♀</span> },
  { name: 'אחר / מעדיף לא לציין', icon: <Sparkles size={18} className="text-brand-muted" /> }
];
  ...
  return (
    <div className="bg-surface min-h-screen relative font-sans overflow-x-hidden" dir="rtl">
      <FadeIn className="relative w-full flex flex-col pb-32">
        ...
        {/* HEADER & COVER UPLOAD */}
        <div className="absolute top-0 left-0 w-full h-[240px] bg-surface-card z-0 overflow-hidden border-b border-surface-border">
        ...
        <div className="relative z-10 w-full pt-[200px] px-2">
          <div className="bg-surface rounded-t-[40px] w-full min-h-screen flex flex-col items-center shadow-[0_-20px_40px_rgba(0,0,0,0.7)]">
            ...
            <motion.div whileHover={{ scale: 1.05 }} className="w-[110px] h-[110px] rounded-full bg-surface shadow-xl p-1.5 relative -mt-[55px] z-20 mb-8">
              ...
              <button 
                ...
                className="absolute bottom-0 right-0 w-10 h-10 bg-accent-primary text-white rounded-full flex items-center justify-center shadow-lg border-2 border-surface active:scale-90 transition-all z-20 disabled:opacity-50"
              >
            ...
            <div className="w-full flex flex-col gap-6 px-1">
              ...
              <div className="bg-surface-card border border-surface-border flex flex-col gap-6 rounded-[32px] p-6 shadow-sm relative overflow-hidden">
                ...
                <div className="flex flex-col gap-4 relative z-10 px-1">
                  ...
                  {/* Signal Price */}
                  <div className="bg-surface border border-surface-border rounded-[24px] p-4 flex items-center justify-between shadow-sm mb-2">
                  ...
                  <div className="flex flex-col gap-1.5 text-right bg-surface border border-surface-border rounded-[20px] p-3 shadow-sm focus-within:border-accent-primary/50 transition-colors">
                    <label className="text-brand-muted text-[11px] font-black tracking-widest uppercase flex items-center justify-start gap-2">
                      <MessageSquare size={14} className="text-accent-primary" /><span>ביו</span>
                    </label>
                ...
                <div className="flex justify-end mt-4 relative z-10">
                  <Button onClick={handleSaveDetails} disabled={savingDetails} className="w-full bg-accent-primary text-white rounded-[20px] font-black text-[14px] uppercase tracking-widest h-14 shadow-lg active:scale-[0.98] transition-all">
                    {savingDetails ? <Loader2 size={20} className="animate-spin text-white" /> : 'שמור שינויים'}
                  </Button>
                </div>
              </div>
              ...
              <div className="bg-surface-card backdrop-blur-xl border border-surface-border flex flex-col gap-6 rounded-[32px] p-7 shadow-sm relative overflow-hidden mb-6">
                ...
                <div className="flex justify-end mt-4 relative z-10">
                  <Button onClick={handleUpdatePassword} disabled={updatingPassword || !passwordData.new_password} className="w-full bg-surface border border-surface-border text-brand rounded-[20px] font-black text-[14px] uppercase tracking-widest h-14 shadow-sm disabled:opacity-50 active:scale-[0.98] transition-all hover:bg-white/5">
      ...
      {mounted && portalNode && createPortal(
        <>
            ...
            {showZodiacPicker && (
              <div className="fixed inset-0 z-[999999] flex flex-col justify-end" dir="rtl">
                ...
                <motion.div
                  ...
                  className="relative z-10 bg-surface rounded-t-[40px] p-6 pb-[env(safe-area-inset-bottom,32px)] border-t border-surface-border max-h-[85vh] flex flex-col shadow-[0_-20px_50px_rgba(0,0,0,0.8)]"
                >
                  ...
                  <div className="flex-1 overflow-y-auto grid grid-cols-2 gap-3 scrollbar-hide pr-1">
                    {ZODIAC_SIGNS.map((sign) => (
                      <button key={sign.name} onClick={() => { setFormData(p => ({ ...p, zodiac: sign.name })); triggerFeedback('pop'); setShowZodiacPicker(false); }} className={`flex items-center justify-center gap-3 p-4 rounded-[20px] border transition-all ${formData.zodiac === sign.name ? 'bg-accent-primary border-accent-primary text-white shadow-md' : 'bg-surface-card border-surface-border text-brand hover:border-accent-primary/30'}`}>
                ...
                >
                  ...
                  <div className="flex-1 overflow-y-auto flex flex-col gap-3 scrollbar-hide pr-1">
                    {RELATIONSHIP_STATUSES.map((status) => (
                      <button key={status.name} onClick={() => { setFormData(p => ({ ...p, relationship_status: status.name })); triggerFeedback('pop'); setShowRelationshipPicker(false); }} className={`flex items-center justify-center p-4 rounded-[20px] border transition-all ${formData.relationship_status === status.name ? 'bg-accent-primary border-accent-primary text-white shadow-md' : 'bg-surface-card border-surface-border text-brand hover:border-accent-primary/30'}`}>
          ...
          <AnimatePresence>
            {showGenderPicker && (
              <div className="fixed inset-0 z-[999999] flex flex-col justify-end" dir="rtl">
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => setShowGenderPicker(false)} />
                <motion.div
                  drag="y" dragConstraints={{ top: 0, bottom: 0 }} dragElastic={0.2}
                  onDragEnd={(e, info) => { if (info.offset.y > 100 || info.velocity.y > 500) setShowGenderPicker(false); }}
                  initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
                  transition={{ type: 'spring', damping: 25, stiffness: 400 }}
                  className="relative z-10 bg-surface rounded-t-[40px] p-6 pb-[env(safe-area-inset-bottom,32px)] border-t border-surface-border max-h-[85vh] flex flex-col shadow-[0_-20px_50px_rgba(0,0,0,0.8)]"
                >
                  <div className="w-16 h-1.5 bg-white/10 rounded-full mx-auto mb-6 shrink-0 cursor-grab active:cursor-grabbing" />
                  <div className="flex justify-center items-center mb-6 shrink-0">
                    <h3 className="text-brand font-black text-lg">בחר מין</h3>
                  </div>
                  <div className="flex-1 overflow-y-auto flex flex-col gap-3 scrollbar-hide pr-1">
                    {GENDER_OPTIONS.map((gender) => (
                      <button key={gender.name} onClick={() => { setFormData(p => ({ ...p, gender: gender.name })); triggerFeedback('pop'); setShowGenderPicker(false); }} className={`flex items-center gap-3 justify-center p-4 rounded-[20px] border transition-all ${formData.gender === gender.name ? 'bg-accent-primary border-accent-primary text-white shadow-md' : 'bg-surface-card border-surface-border text-brand hover:border-accent-primary/30'}`}>
                        {formData.gender === gender.name ? <span className="text-white">{gender.icon}</span> : gender.icon}
                        <span className="font-black text-[15px]">{gender.name}</span>
                      </button>
                    ))}
                  </div>
                </motion.div>
...
const InputField: React.FC<InputFieldProps> = ({ label, icon: Icon, value, onChange, placeholder, type = 'text', dir = 'rtl' }) => (
  <div className="flex flex-col gap-1.5 text-right bg-surface border border-surface-border rounded-[20px] p-3 shadow-sm focus-within:border-accent-primary/50 transition-colors">
    <label className="text-brand-muted text-[11px] font-black tracking-widest uppercase flex items-center justify-start gap-2">
      <Icon size={14} className="text-accent-primary" /><span>{label}</span>
    </label>
    <input type={type} value={value} onChange={e => onChange(e.target.value)} className="bg-transparent border-none outline-none text-brand text-[15px] font-bold placeholder:text-brand-muted/40 px-1" placeholder={placeholder} dir={dir} style={{ colorScheme: 'dark' }} />
  </div>
);

// CLEAN FLOATING SELECT FIELD
...
const SelectField: React.FC<SelectFieldProps> = ({ label, icon: Icon, value, placeholder, onClick }) => (
  <div className="flex flex-col gap-1.5 text-right bg-surface border border-surface-border rounded-[20px] p-3 shadow-sm cursor-pointer hover:border-accent-primary/30 transition-colors" onClick={onClick}>
    <label className="text-brand-muted text-[11px] font-black tracking-widest uppercase flex items-center justify-start gap-2">
      <Icon size={14} className="text-accent-primary" /><span>{label}</span>
    </label>
