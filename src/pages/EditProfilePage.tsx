  ...
  return (
    <FadeIn className="bg-[#0d0d0f] min-h-[100dvh] font-sans text-white overflow-x-hidden relative pb-24" dir="rtl">
      ...
      <div className="relative w-full h-[350px]">
        ...
        {/* Cover Edit Button - Circular, Glass, Fixed at bottom-left corner of cover */}
        <button onClick={() => coverInputRef.current?.click()} disabled={uploadingCover} className="absolute bottom-6 left-6 z-20 w-10 h-10 rounded-full bg-white/5 backdrop-blur-xl border border-white/10 flex items-center justify-center text-white/50 hover:text-white transition-all disabled:opacity-50">
          {uploadingCover ? <Loader2 size={16} className="animate-spin" /> : <Camera size={16} strokeWidth={2.5} />}
        </button>
      </div>

      {/* 📝 Elegant Form */}
      <div className="px-6 -mt-[190px] relative z-20 flex flex-col gap-3 max-w-lg mx-auto">
        
        {/* Center Avatar (Floating inside the cover area) */}
        <div className="relative group cursor-pointer w-28 h-28 mx-auto mb-16" onClick={() => avatarInputRef.current?.click()}>
          <div className="w-full h-full rounded-full border-2 border-[#0d0d0f] overflow-hidden bg-[#111] shadow-[0_10px_30px_rgba(0,0,0,0.8)] relative">
          ...
          {/* Small camera badge - Transparent with White outline */}
          <div className="absolute bottom-1 right-1 w-8 h-8 rounded-full bg-transparent border-2 border-white/20 flex items-center justify-center shadow-md pointer-events-none text-white/80">
            <Camera size={12} strokeWidth={3} />
          </div>
        </div>

        {/* Signal Price Upgraded - Floating accent glass */}
        <div className="bg-white/[0.04] backdrop-blur-md hover:bg-white/[0.06] border border-accent-primary/20 rounded-2xl p-4 flex items-center justify-between shadow-lg transition-colors mb-2">
          <div className="flex flex-col">
            <span className="text-accent-primary font-black text-[13px] flex items-center gap-1.5 drop-shadow-[0_0_5px_rgba(var(--color-accent-primary),0.3)]"><Zap size={14} className="text-amber-400" />תעריף סיגנל</span>
            <span className="text-white/40 text-[10px] font-bold mt-0.5 pl-1">עלות שליחת הודעה אליך</span>
          </div>
