...
export const NotificationsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchNotifs = async () => {
      try {
        ...
        // מושכים התראות ישירות מ-Supabase! עוקפים את השרת לגמרי
        ...
        // סמנים כנקראו ישירות במסד הנתונים
  ...
  const getNotificationStyle = (type: string, isRead: boolean) => {
    ...
    return { 
      ...
      color: type === 'like' ? 'text-[#e91e63]' : type === 'comment' ? 'text-[#2196f3]' : type === 'gift' ? 'text-[#ff9800]' : 'text-[#8bc34a]', 
      bg: 'bg-white/10', 
      ...
      <div className="flex items-center justify-between mb-8 relative z-10 px-1">
        ...
        <div className="flex flex-col items-center">
          <h1 className="text-2xl font-black text-white flex items-center gap-2 drop-shadow-[0_0_15px_rgba(255,255,255,0.2)]">
            <Bell size={20} className="text-[#3f51b5]" /> התראות
          </h1>
        ...
        <AnimatePresence>
          {notifications.length === 0 ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center py-32 gap-5">
              <motion.div animate={{ rotate: [0, -10, 10, -10, 10, 0] }} transition={{ repeat: Infinity, duration: 4, repeatDelay: 2 }} className="w-24 h-24 rounded-full bg-black border border-white/10 flex items-center justify-center shadow-2xl">
            ...
            notifications.map((notif, idx) => {
              ...
              return (
                <motion.div key={notif.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }} onClick={() => handleNotificationClick(notif)} className={notif.action_url ? 'cursor-pointer' : ''}>
                  <div className={`p-5 rounded-[28px] transition-all flex gap-4 items-start bg-white/[0.06] backdrop-blur-xl border ${style.border} ${notif.action_url ? 'hover:bg-white/[0.08] active:scale-[0.98]' : ''}`}>
                    ...
                    <div className="flex flex-col flex-1 text-right mt-1">
                      <h3 className="text-white font-black text-[16px] mb-1.5 flex items-center justify-between">
                        {notif.title}
                        {!notif.is_read && <span className="w-2.5 h-2.5 bg-[#e91e63] rounded-full shadow-[0_0_10px_#e91e63] shrink-0 ml-1"></span>}
                      </h3>
