...
export const ProfilePage: React.FC = () => {
  ...
  const handleFollowToggle = async () => {
    ...
    try {
      ...
      await apiFetch(`/api/profile/${targetId}/follow`, {
        method: 'POST',
  ...
  const handleLike = async (postId: string, isLiked: boolean) => {
    ...
    setData((prev: any) => ({
      ...
      savedPosts: update(prev.savedPosts || []),
    }));
  ...
  const submitComment = async () => {
    ...
    try {
      if (editingCommentId) {
        ...
        setComments((prev) =>
          prev.map((c) => (c.id === editingCommentId ? { ...c, content: newComment.trim() } : c))
        );
        setEditingCommentId(null);
      } else {
        const payload: any = {
  ...
  const deletePost = async (postId: string) => {
    ...
    if (fullScreenMedia) {
      setFullScreenMedia((curr) => curr?.filter((p) => p.id !== postId) || null);
    }

    setData((prev: any) => ({
      ...prev,
      posts: (prev.posts || []).filter((p: any) => p.id !== postId),
      savedPosts: (prev.savedPosts || []).filter((p: any) => p.id !== postId),
    }));
  ...
  const saveEditedPost = async () => {
    ...
    try {
      ...
      const update = (list: any[]) =>
        list.map((p) => (p.id === editingPost.id ? { ...p, content: editPostText.trim() } : p));
      ...
      if (fullScreenMedia) {
        setFullScreenMedia((prev) => (prev ? update(prev) : prev));
      }
  ...
  const renderCommentText = (text: string) => {
    ...
    return parts.map((part, i) =>
      part.startsWith('@') ? (
        <span key={i} className="text-[#2196f3] font-bold">
          {part}
        </span>
      ) : (
  ...
  const displayLink = userProfile?.social_link
    ? userProfile.social_link.replace(/^https?:\/\/(www\.)?/, '').replace(/\/$/, '')
    : '';
  ...
  const aboutItems = [
    {
      ...
      icon: <MapPin size={16} className="text-[#2196f3]" />,
    },
    {
      key: 'relationship_status',
      label: 'מצב משפחתי',
      value: userProfile?.relationship_status,
      icon: <Heart size={16} className="text-[#ff6b6b]" />,
    },
    {
      key: 'education',
      label: 'השכלה',
      value: userProfile?.education,
      icon: <GraduationCap size={16} className="text-[#d500f9]" />,
    },
    {
      key: 'job_title',
      label: 'עיסוק',
      value: userProfile?.job_title,
      icon: <Briefcase size={16} className="text-[#8bc34a]" />,
    },
    {
      key: 'birth_date',
      label: 'תאריך לידה',
      value: userProfile?.birth_date ? new Date(userProfile.birth_date).toLocaleDateString('he-IL') : '',
      icon: <Calendar size={16} className="text-[#ffc107]" />,
    },
    {
      key: 'bio',
      label: 'ביו',
      value: userProfile?.bio,
      icon: <Sparkles size={16} className="text-white/60" />,
      full: true,
    },
    ...
    <div className="bg-[#0C0C0C] min-h-screen relative font-sans" dir="rtl">
      ...
      >
        ...
        <div className="absolute top-6 right-5 flex flex-col items-center">
          ...
          <span className="text-[#e5e4e2] font-black text-[22px] leading-none drop-shadow-[0_0_12px_rgba(229,228,226,0.5)]">
            {currentLevel}
          </span>
        </div>
      </motion.div>
      ...
      <FadeIn className="relative z-10 pt-[170px] pb-32">
        <div className="bg-[#0C0C0C]/90 backdrop-blur-3xl rounded-t-[40px] px-4 min-h-screen flex flex-col items-center pt-0 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] border-t border-white/5">
          ...
          <div className="text-center mt-3 w-full">
            <h2 className="text-[20px] font-black text-white tracking-tight">{userProfile?.full_name || 'משתמש'}</h2>
            <p className="text-white/30 font-bold text-[12px] tracking-widest mb-4" dir="ltr">
            ...
            <div className="flex items-center justify-center gap-2 text-[13px] text-white/50 font-medium mb-5 w-full max-w-[280px] mx-auto flex-wrap">
              <span className="cursor-pointer hover:text-white transition-colors" onClick={() => openUsersListSheet('followers')}>
                עוקבים <span className="font-black text-white">{followersCount}</span>
              </span>
              <span>•</span>
              <span className="cursor-pointer hover:text-white transition-colors" onClick={() => openUsersListSheet('following')}>
                נעקבים <span className="font-black text-white">{followingCount}</span>
              </span>
              <span>•</span>
              <span>
                מוניטין <span className="font-black text-white">{trueReputation}</span>
              </span>
            </div>
            ...
            {!isMyProfile && (
              <div className="flex justify-center gap-3 mb-6 w-full px-4">
                <Button
                  ...
                  className={`flex-1 h-12 rounded-full font-black text-[14px] flex items-center justify-center transition-all active:scale-95 disabled:opacity-50 ${
                    isFollowing
                ...
                <Button
                  ...
                  className="flex-1 h-12 bg-white text-black rounded-full font-black text-[14px] flex items-center justify-center transition-all active:scale-95 shadow-lg"
                >
                  הודעה
                </Button>
              </div>
            ...
            {(userProfile?.zodiac || userProfile?.social_link || aboutItems.length > 0) && (
              <div className="flex flex-col items-center gap-2 mb-5 w-full px-4">
                {userProfile?.social_link && (
                  <a
                    ...
                    className="text-[#e5e4e2] text-[12px] font-bold flex items-center gap-1.5 hover:text-white transition-colors"
                  >
                    <span dir="ltr" className="tracking-wide">
                      {displayLink}
                    </span>
                    <LinkIcon size={12} className="text-[#e5e4e2]/60" />
                  </a>
                )}

                {userProfile?.zodiac && (
                  <span className="text-white/40 text-[12px] font-medium">{userProfile.zodiac}</span>
                )}
                ...
                {aboutItems.length > 0 && (
                  <div className="w-full max-w-[360px]">
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 mt-1 text-white/75 hover:text-white transition-colors"
                    >
                      <span className="text-[12px] font-black tracking-wide">אודות</span>
                    ...
                    <AnimatePresence initial={false}>
                      {aboutOpen && (
                        ...
                        >
                          <div className="mt-3 bg-white/[0.04] border border-white/10 rounded-[24px] p-4 backdrop-blur-xl text-right shadow-[0_10px_30px_rgba(0,0,0,0.25)]">
                            <div className="grid grid-cols-1 gap-3">
                              {aboutItems.map((item: any) => (
                                <div
                                  key={item.key}
                                  className={`flex items-start gap-3 rounded-[18px] border border-white/6 bg-white/[0.03] px-4 py-3 ${
                                    item.full ? 'flex-col items-stretch' : ''
                                  }`}
                                >
                                  {!item.full ? (
                                    <>
                                      ...
                                      <div className="flex-1">
                                        <div className="text-white/35 text-[10px] font-black tracking-widest mb-1 uppercase">
                                          {item.label}
                                        </div>
                                        <div className="text-white/90 text-[13px] font-medium">{item.value}</div>
                                      </div>
                                    </>
                                  ) : (
                                    <>
                                      <div className="flex items-center gap-2">
                                        {item.icon}
                                        <div className="text-white/35 text-[10px] font-black tracking-widest uppercase">
                                          {item.label}
                                        </div>
                                      </div>
                                      <div className="text-white/90 text-[13px] font-medium leading-6 whitespace-pre-wrap">
                                        {item.value}
                                      </div>
                                    </>
                                  )}
          ...
          <div className="w-full flex flex-col gap-3 px-2 mb-5">
            <div className="flex justify-between items-center w-full">
              <span className="text-white/50 text-[12px] font-bold">
                רצף: <span className="text-white">{streak} ימים</span>
              </span>
              {isMyProfile && (
                <span className="text-white/50 text-[12px] font-bold">
                  <span className="text-white">{currentXP}</span> / {xpToNextLevel}
                </span>
              )}
            </div>
            ...
            {isMyProfile && (
              <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden relative shadow-inner">
                <motion.div
                  ...
                  className="absolute top-0 right-0 h-full bg-gradient-to-l from-[#ff5722] via-[#d500f9] to-[#2196f3] rounded-full"
                />
              </div>
            )}
          </div>
            ...
            <div className="flex w-full mb-5 bg-white/5 p-1 rounded-full border border-white/5 shadow-inner">
              <button
                ...
                className={`flex-1 py-3 text-[13px] font-black transition-all rounded-full flex items-center justify-center ${
                  activeTab === 'posts' ? 'bg-[#1A1C20] text-white shadow-md' : 'text-white/40 hover:text-white/70'
                }`}
              >
                פוסטים
              </button>
              ...
              <button
                ...
                className={`flex-1 py-3 text-[13px] font-black transition-all rounded-full flex items-center justify-center ${
                  activeTab === 'joined' ? 'bg-[#1A1C20] text-white shadow-md' : 'text-white/40 hover:text-white/70'
                }`}
              >
                מועדונים
              </button>
              ...
              {isMyProfile && (
                <button
                  ...
                  className={`flex-1 py-3 text-[13px] font-black transition-all rounded-full flex items-center justify-center ${
                    activeTab === 'saved' ? 'bg-[#1A1C20] text-white shadow-md' : 'text-white/40 hover:text-white/70'
                  }`}
                >
                  שמורים
                </button>
                  ...
                  {userPosts.length === 0 ? (
                    ...
                    userPosts.map((post: any) => (
                      <div
                        ...
                        onClick={() => {
                          openOverlay(() => {
                            const first = { ...post, _uid: `${post.id}-${Math.random().toString(36).slice(2)}` };
                            const rest = mediaPosts.filter((p: any) => p.id !== post.id).map((p: any) => ({
                              ...p,
                              _uid: `${p.id}-${Math.random().toString(36).slice(2)}`,
                            }));
                            setFullScreenMedia([first, ...rest]);
                          });
                        }}
                        ...
                        {post.media_url ? (
                          post.media_type === 'video' ? (
                            ...
                            <img
                              src={post.media_url}
                              onError={(e) => {
                                e.currentTarget.src = 'https://placehold.co/500x500/111/333?text=Image';
                              }}
                              className="w-full h-full object-cover"
                            />
                          )
                        ) : (
                          <div className="w-full h-full flex items-center justify-center p-2 text-center text-white/50 text-[9px] line-clamp-3">
                            {post.content}
                          </div>
                        )}
                      </div>
                ...
                <motion.div key="joined" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-3 gap-1">
                  {joinedCircles.map((circle: any) => (
                    <div
                      ...
                      className="aspect-square bg-[#111] relative overflow-hidden cursor-pointer border border-white/5 rounded-[24px]"
                    >
                      {circle.cover_url ? (
                        <img
                      ...
                      <div className="absolute bottom-0 w-full p-1 bg-black/60 backdrop-blur-sm text-center">
                        <span className="text-white text-[9px] font-bold line-clamp-1">{circle.name}</span>
                      </div>
                    </div>
                  ))}
                </motion.div>
                  ...
                  {userSavedPosts.length === 0 ? (
                    ...
                    userSavedPosts.map((post: any) => (
                      <div
                        key={post.id}
                        onClick={() => {
                          const savedMedia = userSavedPosts.filter((p: any) => p.media_url);
                          openOverlay(() => {
                            const first = { ...post, _uid: `${post.id}-${Math.random().toString(36).slice(2)}` };
                            const rest = savedMedia.filter((p: any) => p.id !== post.id).map((p: any) => ({
                              ...p,
                              _uid: `${p.id}-${Math.random().toString(36).slice(2)}`,
                            }));
                            setFullScreenMedia([first, ...rest]);
          ...
          <>
            <AnimatePresence>
                        ...
                        <div key={keyVal} className="w-full h-screen snap-center relative bg-black flex items-center justify-center">
                          {isVid ? (
                            <video
                              ...
                              loop
                              playsInline
                              className="w-full h-full object-cover profile-full-media-item"
                              onClick={(e) => (e.currentTarget.paused ? e.currentTarget.play() : e.currentTarget.pause())}
                            />
                          ) : (
                            <img
                              src={vid.media_url}
                              className="w-full h-full object-contain profile-full-media-item"
                              onError={(e) => {
                          ...
                          <div className="absolute bottom-48 left-4 flex flex-col gap-6 items-center z-50">
                            <button onClick={(e) => { e.stopPropagation(); handleLike(vid.id, vid.is_liked); }} className="flex flex-col items-center gap-1 active:scale-90 transition-transform">
                              <Heart size={30} className={vid.is_liked ? 'text-[#e91e63]' : 'text-white'} fill={vid.is_liked ? 'currentColor' : 'none'} strokeWidth={1.5} />
                          ...
                          <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/90 via-black/40 to-transparent flex flex-col justify-end pointer-events-none">
                            <div
                              className="flex items-center gap-3 mb-2 cursor-pointer w-fit pr-2 pointer-events-auto"
                              onClick={(e) => {
                                e.stopPropagation();
                                closeOverlay();
                            ...
                            >
                              <div className="w-12 h-12 rounded-full overflow-hidden bg-black border-2 border-white/20 shrink-0 shadow-lg">
                                {vid.profiles?.avatar_url ? (
                                  <img src={vid.profiles.avatar_url} className="w-full h-full object-cover" />
                                ) : (
                            ...
                            <p
                              className="text-white/90 text-[15px] font-medium text-right pr-2 w-5/6 line-clamp-3 pointer-events-auto cursor-pointer"
                              onClick={(e) => {
                                e.stopPropagation();
              ...
              {activeCommentsPostId && (
                <div className="fixed inset-0 z-[100000] flex flex-col justify-end" dir="rtl" onTouchStart={stopPropagation} onTouchMove={stopPropagation}>
                  ...
                  <motion.div
                    ...
                    onDragEnd={(e, info) => { if (info.offset.y > 100) closeOverlay(); }}
                    initial={{ y: '100%' }}
                    animate={{ y: 0 }}
                    exit={{ y: '100%' }}
                    transition={{ type: 'spring', damping: 25, stiffness: 300 }}
                    className="relative z-10 bg-[#0F0F0F] rounded-t-[40px] h-[85vh] flex flex-col overflow-hidden pb-10 shadow-[0_-20px_50px_rgba(0,0,0,0.8)] border-t border-white/10"
                  >
                    <div className="w-full py-5 flex justify-center cursor-grab active:cursor-grabbing border-b border-white/5" onPointerDown={(e) => commentsDragControls.start(e)} style={{ touchAction: 'none' }}>
                    ...
                    <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6" onPointerDown={stopPropagation} onTouchStart={stopPropagation}>
                      {loadingComments ? (
                        <Loader2 className="animate-spin mx-auto text-white/20 mt-10" />
                      ) : (
                        comments
                            ...
                            return (
                              <div key={c.id} className="flex flex-col gap-2">
                                <div className="flex gap-3">
                                  <div
                                    className="w-10 h-10 min-w-[40px] rounded-full bg-[#111] shrink-0 overflow-hidden cursor-pointer border border-white/10 flex items-center justify-center"
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      closeOverlay();
                                  ...
                                  >
                                    {c.profiles?.avatar_url ? (
                                      <img src={c.profiles.avatar_url} className="w-full h-full object-cover object-center" />
                                    ) : (
                                  ...
                                  <div className="flex flex-col flex-1">
                                    <div className="bg-[#111] p-3 rounded-[24px] rounded-tr-sm cursor-pointer" onClick={() => openOverlay(() => setCommentActionModal(c))}>
                                      <span
                                        className="text-white/90 font-bold text-xs mb-1 inline-block"
                                        onClick={(e) => {
                                          e.stopPropagation();
                                          closeOverlay();
                                          setTimeout(() => navigate(`/profile/${c.user_id}`), 50);
                                    ...
                                    <div className="flex items-center gap-4 mt-2 px-2">
                                      <span
                                        className="text-[11px] text-white/40 cursor-pointer font-medium hover:text-white"
                                        onClick={() => {
                                          setReplyingTo(c);
                                      ...
                                      <button onClick={() => toggleCommentLike(c.id)} className={`ml-auto flex items-center gap-1 ${likedComments.has(c.id) ? 'text-[#e91e63]' : 'text-white/30'}`}>
                                    ...
                                    {replies.length > 0 && (
                                      <button
                                        onClick={() => setExpandedThreads((prev) => ({ ...prev, [c.id]: !prev[c.id] }))}
                                        className="text-left text-[11px] font-bold text-white/40 hover:text-white/70 mt-2 flex items-center gap-1"
                                      >
                                ...
                                {isThreadExpanded &&
                                  replies.map((reply) => (
                                    <div key={reply.id} className="flex gap-3 pr-10 mt-2 relative">
                                      ...
                                      <div
                                        className="w-8 h-8 min-w-[32px] rounded-full bg-[#111] shrink-0 overflow-hidden cursor-pointer z-10 border border-white/10 flex items-center justify-center"
                                        onClick={(e) => {
                                          e.stopPropagation();
                                          closeOverlay();
                                      ...
                                      >
                                        {reply.profiles?.avatar_url ? (
                                          <img src={reply.profiles.avatar_url} className="w-full h-full object-cover object-center" />
                                        ) : (
                                          <UserCircle className="w-full h-full p-1.5 text-white/20" />
                                        )}
                                      </div>
                                      ...
                                      <div className="flex flex-col flex-1 z-10">
                                        <div className="bg-[#111] p-3 rounded-[24px] rounded-tr-sm cursor-pointer" onClick={() => openOverlay(() => setCommentActionModal(reply))}>
                                          <span
                                            className="text-white/90 font-bold text-[11px] mb-1 inline-block"
                                            onClick={(e) => {
                                              e.stopPropagation();
                                              closeOverlay();
                                              setTimeout(() => navigate(`/profile/${reply.user_id}`), 50);
                                        ...
                                        <div className="flex items-center gap-4 mt-2 px-2">
                                          <span
                                            className="text-[10px] text-white/40 cursor-pointer font-medium hover:text-white"
                                            onClick={() => {
                                              setReplyingTo(c);
                                          ...
                                          <button onClick={() => toggleCommentLike(reply.id)} className={`ml-auto flex items-center gap-1 ${likedComments.has(reply.id) ? 'text-[#e91e63]' : 'text-white/30'}`}>
                    ...
                    <div className="p-4 border-t border-white/10 flex flex-col gap-2 bg-[#0A0A0A]" onPointerDown={stopPropagation}>
                      {replyingTo && !editingCommentId && (
                        <div className="text-[11px] text-[#2196f3] flex items-center justify-between px-3 py-1 bg-[#2196f3]/10 rounded-full w-fit mb-1">
                          <span className="font-bold mr-1">משיב ל-@{replyingTo.profiles?.full_name}</span>
                          <X size={12} className="cursor-pointer" onClick={() => { setReplyingTo(null); setNewComment(''); }} />
                        </div>
                      )}

                      {editingCommentId && (
                        <div className="text-[10px] text-[#2196f3] flex justify-between px-2">
                          <span>עורך תגובה...</span>
                          <span onClick={() => { setEditingCommentId(null); setNewComment(''); }} className="cursor-pointer font-bold">ביטול</span>
                        </div>
                      )}
                      ...
                      <div className="flex gap-2 items-center bg-white/5 rounded-full p-1 pl-2 border border-white/10">
                        <input
                          ...
                          className="flex-1 bg-transparent px-4 text-white text-sm outline-none placeholder:text-white/40"
                        />
                        <button onClick={submitComment} disabled={!newComment.trim()} className="w-9 h-9 bg-[#2196f3] rounded-full flex items-center justify-center text-white active:scale-95 disabled:opacity-50 transition-opacity">
              ...
              {optionsMenuPost && (
                <div className="fixed inset-0 z-[100000] flex flex-col justify-end" onTouchStart={stopPropagation} onTouchMove={stopPropagation}>
                  ...
                  <motion.div
                    ...
                    animate={{ y: 0 }}
                    exit={{ y: '100%' }}
                    className="relative z-10 bg-[#0A0A0A] rounded-t-[40px] p-6 flex flex-col gap-2 pb-12 shadow-[0_-10px_50px_rgba(0,0,0,0.8)] border-t border-white/10"
                  >
                    ...
                    <button
                      onClick={async () => {
                        try {
                          ...
                          toast.error('הפוסט כבר שמור אצלך');
                        }
                        closeOverlay();
                      }}
                          ...
                          onClick={() => {
                            ...
                            setTimeout(() => {
                              openOverlay(() => {
                                ...
                                setEditPostText(optionsMenuPost.content || '');
                                setShowCreatePost(true);
                              });
                            }, 100);
                          }}
                          className="w-full p-4 bg-white/5 rounded-full text-white/90 font-bold flex justify-between items-center text-lg active:bg-white/10 transition-colors mt-4"
                        >
                          <span>ערוך פוסט</span>
              ...
              {commentActionModal && (
                <div className="fixed inset-0 z-[100000] flex flex-col justify-end" onTouchStart={stopPropagation} onTouchMove={stopPropagation}>
                  ...
                  <motion.div
                    ...
                    animate={{ y: 0 }}
                    exit={{ y: '100%' }}
                    className="relative z-10 bg-[#0A0A0A] rounded-t-[40px] p-6 flex flex-col gap-3 pb-12 shadow-[0_-10px_50px_rgba(0,0,0,0.8)] border-t border-white/10"
                  >
                    <div className="w-full py-4 flex justify-center cursor-grab active:cursor-grabbing">
                      <div className="w-16 h-1.5 bg-white/20 rounded-full" />
                    </div>
                    ...
                    <button
                      ...
                      className="w-full p-4 bg-white/5 rounded-full text-white/90 font-bold flex justify-between items-center text-lg hover:bg-white/10 transition-colors"
                    >
                      <span>השב לתגובה</span>
                      <Reply size={20} className="text-white/60" />
                    </button>
                    ...
                    {commentActionModal.user_id === currentUserId && (
                      <>
                        ...
                        <button
                          ...
                          className="w-full p-4 bg-red-500/10 border border-red-500/20 rounded-full text-red-500 font-bold flex justify-between items-center text-lg mt-2 hover:bg-red-500/20 transition-colors"
                        >
                          <span>מחק תגובה</span>
                          <Trash2 size={20} className="text-red-500/80" />
                        </button>
              ...
              {activeDescPost && (
                <div className="fixed inset-0 z-[100000] flex flex-col justify-end" onTouchStart={stopPropagation} onTouchMove={stopPropagation}>
                  ...
                  <motion.div
                    ...
                    animate={{ y: 0 }}
                    exit={{ y: '100%' }}
                    className="relative z-10 bg-[#0A0A0A] rounded-t-[40px] flex flex-col overflow-hidden pb-10 max-h-[75vh] shadow-[0_-10px_50px_rgba(0,0,0,0.8)] border-t border-white/10"
                  >
                    <div className="w-full py-6 flex justify-center cursor-grab active:cursor-grabbing border-b border-white/5">
                      <div className="w-16 h-1.5 bg-white/20 rounded-full" />
                    </div>
                    <div className="px-6 py-4 border-b border-white/5">
                      <h2 className="text-white font-black text-lg text-center">תיאור מלא</h2>
                    </div>
                    <div className="p-6 overflow-y-auto" onPointerDown={stopPropagation} onTouchStart={stopPropagation}>
                      <p className="text-white/90 text-[15px] leading-relaxed text-right whitespace-pre-wrap">{activeDescPost.content}</p>
                    </div>
                  </motion.div>
                </div>
              )}
              ...
              {(showFollowersList || showFollowingList) && (
                <div className="fixed inset-0 z-[100000] flex flex-col justify-end" onTouchStart={stopPropagation} onTouchMove={stopPropagation}>
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-black/70 backdrop-blur-sm" onClick={closeOverlay} />
                  <motion.div
                    drag="y"
                    dragConstraints={{ top: 0, bottom: 0 }}
                    onDragEnd={(e, info) => { if (info.offset.y > 100) closeOverlay(); }}
                    ...
                    animate={{ y: 0 }}
                    exit={{ y: '100%' }}
                    className="relative z-10 bg-[#0A0A0A] rounded-t-[40px] p-6 pb-10 max-h-[75vh] border-t border-white/10 shadow-[0_-10px_50px_rgba(0,0,0,0.8)]"
                  >
                    <div className="w-full py-4 flex justify-center cursor-grab active:cursor-grabbing" onPointerDown={(e) => (showFollowersList ? followersDragControls.start(e) : followingDragControls.start(e))} style={{ touchAction: 'none' }}>
                      <div className="w-16 h-1.5 bg-white/20 rounded-full" />
                    </div>

                    <h3 className="text-white text-lg font-black text-center mb-5">
                      {showFollowersList ? 'עוקבים' : 'נעקבים'}
                    </h3>

                    <div className="overflow-y-auto max-h-[58vh] flex flex-col gap-3">
                      {loadingUsersList ? (
                        <Loader2 className="animate-spin mx-auto text-white/20 mt-6" />
                      ) : usersListData.length === 0 ? (
                        <div className="text-center text-white/35 text-sm py-10">אין נתונים להצגה</div>
                      ) : (
                        usersListData.map((item: any) => (
                          <button
                            ...
                            onClick={() => {
                              closeOverlay();
                              setTimeout(() => navigate(`/profile/${item.username || item.id}`), 50);
                            }}
                            className="w-full flex items-center gap-3 bg-white/[0.04] border border-white/10 rounded-[22px] p-3 active:scale-[0.99] transition-transform"
                          >
                            <div className="w-12 h-12 rounded-full overflow-hidden bg-[#111] border border-white/10 shrink-0">
                              {item.avatar_url ? (
                            ...
                            <div className="flex-1 text-right">
                              <div className="text-white font-black text-[14px]">{item.full_name || 'משתמש'}</div>
                              <div className="text-white/35 text-[11px]" dir="ltr">
                                @{item.username || 'user'}
                              </div>
                            </div>
                          </button>
              ...
              {showCreatePost && (
                <div className="fixed inset-0 z-[100000] flex flex-col justify-end" onTouchStart={stopPropagation} onTouchMove={stopPropagation}>
                  ...
                  <motion.div
                    ...
                    animate={{ y: 0 }}
                    exit={{ y: '100%' }}
                    className="relative z-10 bg-[#0A0A0A] rounded-t-[40px] p-6 flex flex-col gap-4 pb-12 border-t border-white/10 shadow-[0_-10px_50px_rgba(0,0,0,0.8)]"
                  >
                    <div className="w-full py-4 flex justify-center cursor-grab active:cursor-grabbing">
                      <div className="w-16 h-1.5 bg-white/20 rounded-full" />
                    </div>
                    ...
                    <div className="flex justify-between items-center">
                      <h3 className="text-white font-black text-lg">עריכת פוסט</h3>
                      <button onClick={closeOverlay} className="text-white/40">
                        <X size={20} />
                      </button>
                    </div>
                    ...
                    <textarea
                      ...
                      className="h-36 bg-[#111] rounded-[24px] p-5 text-white text-[16px] outline-none resize-none border border-white/10 placeholder:text-white/40 shadow-inner"
                    />

                    <Button
                      onClick={saveEditedPost}
                      disabled={!editPostText.trim()}
                      className="h-14 bg-[#2196f3] text-white font-black rounded-2xl mt-2 shadow-md"
                    >
                      שמור עריכה
                    </Button>
                  </motion.div>
