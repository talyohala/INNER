...
import { 
  UserCircle, Loader2, Heart, MessageSquare, MoreVertical, Share2, Reply, Trash2, X, Send, Download, Link as LinkIcon, Edit2, Bookmark
} from 'lucide-react';
                ...
                <motion.div key="posts" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-3 gap-1">
                  {userPosts.length === 0 ? (
                    ...
                    userPosts.map((post: any) => (
                      ...
                      }} className="aspect-square bg-white/5 relative overflow-hidden cursor-pointer active:opacity-70 border border-white/5 rounded-[32px]">
              ...
              {activeTab === 'joined' && (
                <motion.div key="joined" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-3 gap-1">
                   {joinedCircles.map((circle: any) => (
                     <div key={circle.id} onClick={() => navigate(`/circle/${circle.slug}`)} className="aspect-square bg-[#111] relative overflow-hidden cursor-pointer border border-white/5 rounded-[32px]">
                  ...
                  <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6 scrollbar-hide">
                      {loadingComments ? <Loader2 className="animate-spin mx-auto text-white/20 mt-10" /> : comments.filter(c => c && !c.parent_id).map((c, i) => (
                          <div key={i} className="flex gap-3">
                              ...
                              <div className="bg-[#111] p-4 rounded-[32px] rounded-tr-sm border border-white/5 flex-1 shadow-inner cursor-pointer" onClick={() => openOverlay(() => setCommentActionModal(c))}>
