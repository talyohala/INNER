...
export const CirclePage: React.FC = () => {
  ...
  const fetchCircleData = async (uid: string) => {
    try {
      ...
      const newData = { circle, isMember, membership, posts: pData || [] };
      setData(newData);
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      if (insertedPost) {
        setData((curr: any) => ({ ...curr, posts: [insertedPost, ...curr.posts] }));
      }
      ...
      <FadeIn className="bg-surface h-[100dvh] font-sans flex flex-col relative overflow-hidden" dir="rtl">
        ...
        <div className="relative w-full h-[200px] shrink-0 bg-surface overflow-hidden flex flex-col justify-end pb-5">
          ...
          <div className="relative z-10 px-5 text-center flex flex-col items-center">
            <h1 className="text-3xl font-black text-brand mb-3 drop-shadow-[0_2px_10px_rgba(0,0,0,0.5)]">{circle.name}</h1>
            ...
            <div className="flex items-center justify-center gap-3 bg-surface-card/80 backdrop-blur-xl border border-white/10 px-5 py-2.5 rounded-full shadow-[0_5px_15px_rgba(0,0,0,0.3)]">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[12px] font-black text-white tracking-widest">{onlineCount} מחוברים</span>
              </div>
              ...
              {isMember && typingUsers.size > 0 && (
                <>
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[11px] font-bold text-white/80">{typingUsers.size} מקלידים...</span>
                  </div>
                </>
              )}
            </div>
        ...
        {!isMember ? (
          <div className="flex-1 flex flex-col items-center justify-center p-6 gap-6 relative z-10">
            ...
            <div className="text-center">
              ...
              <p className="text-brand-muted text-[13px] font-medium max-w-[250px] mx-auto">הצטרף עכשיו כדי לקבל גישה לתוכן בלעדי, כספות ושיחות עם שאר חברי המועדון.</p>
            </div>
            <Button onClick={() => handleJoin('INNER')} disabled={joining} className="w-full h-14 bg-white text-black font-black rounded-full uppercase tracking-widest text-[14px] shadow-[0_5px_20px_rgba(255,255,255,0.2)] active:scale-95 transition-all">
          ...
          <div className="flex flex-col flex-1 overflow-hidden relative">
            ...
            {/* FLOATING TABS (Pill Style) */}
            <div className="flex justify-center px-4 py-2 z-10 relative shrink-0">
              <div className="flex items-center bg-surface-card border border-surface-border rounded-full p-1 shadow-inner w-full max-w-[320px]">
                {['chat', 'vaults', 'members'].map((tab) => (
                  <button
                    key={tab}
                    onClick={() => { triggerFeedback('pop'); setActiveTab(tab as any); }}
                    className={`flex-1 py-2 text-[12px] font-black uppercase tracking-widest transition-all rounded-full ${
                      activeTab === tab
                  ...
                  >
                    {tab === 'chat' ? 'צ׳אט' : tab === 'vaults' ? 'כספות' : 'חברים'}
                  </button>
                ))}
              </div>
            </div>
              ...
              <div className="flex-1 p-4 flex flex-col gap-6 bg-surface overflow-y-auto pb-[120px] relative">
                {loadingVaults ? <Loader2 className="animate-spin text-accent-primary mx-auto my-20" /> : vaults.length === 0 ? (
                  <div className="text-center py-20 opacity-50 flex flex-col items-center">
                    ...
                    <h3 className="text-lg font-black text-brand-muted tracking-widest uppercase">אין כספות עדיין</h3>
                  </div>
                ) : vaults.map(vault => <VaultCard key={vault.id} vault={vault} onUnlockSuccess={fetchVaults} />)}
                
                {isOwner && (
                  <button onClick={() => navigate(`/circle/${slug}/vaults/create`)} className="fixed bottom-[100px] left-6 w-14 h-14 bg-accent-primary text-white rounded-full flex items-center justify-center shadow-[0_5px_20px_rgba(var(--color-accent-primary),0.4)] active:scale-90 transition-transform z-50">
                    <Plus size={28} />
                  </button>
                )}
              </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col text-right">
                        <span className="text-brand font-black text-[15px] flex items-center gap-1.5">
                          ...
                          {m.role === 'admin' && <ShieldAlert size={14} className="text-accent-primary" />}
                        </span>
