...
export const CirclePage: React.FC = () => {
  ...
  const handleJoin = async () => {
    ...
    try {
      if (data.isMember) {
        await supabase.from('circle_members').delete().match({
        ...
        setData((prev: any) => ({
          ...
          circle: {
            ...prev.circle,
            members_count: Math.max((prev.circle.members_count || 1) - 1, 0),
          },
        }));
      } else {
        const res = await apiFetch(`/api/circles/${data.circle.slug}/join`, {
          method: 'POST',
        ...
        if (res) {
          setData((prev: any) => ({
            ...
            circle: {
              ...prev.circle,
              members_count: (prev.circle.members_count || 0) + 1,
            },
          }));
  ...
  const deleteComment = async (commentId: string) => {
    ...
    const update = (list: any[]) =>
      list.map((p) =>
        p.id === activePost?.id ? { ...p, comments_count: Math.max(0, (p.comments_count || 0) - 1) } : p
      );
  ...
  const deletePost = async (postId: string) => {
    ...
    setData((curr: any) => ({
      ...curr,
      posts: curr.posts.filter((p: any) => p.id !== postId),
    }));
  ...
  const handleShare = async (post: any) => {
    ...
    try {
      ...
      if (isNative) {
        await Share.share({
          title: 'INNER',
          text: post.content || 'צפה בפוסט ב-INNER',
          url: publicUrl,
        });
  ...
  const handleDownloadMedia = async (mediaUrl: string) => {
    try {
      ...
      toast.error('שגיאה בהורדה', { id: 'dl' });
    }
    closeOverlay();
  };
    ...
    scrollTimeout.current = setTimeout(() => {
      ...
      if (target.scrollHeight - target.scrollTop <= target.clientHeight * 2 && fullScreenMedia) {
        ...
        if (mediaPosts.length > 0) {
          const more = Array.from({ length: 3 }).map(
            () => mediaPosts[Math.floor(Math.random() * mediaPosts.length)]
          );
          setFullScreenMedia((prev) => [
            ...(prev || []),
            ...more.map((p) => ({ ...p, _uid: Math.random().toString() })),
          ]);
        }
      }
    }, 150);
                  ...
                  <div className="relative mt-2 mb-4 w-fit">
                    {selectedFile.type.startsWith('video/') ? (
                      <video
                        ...
                        controls
                        playsInline
                        className="w-28 h-28 rounded-[20px] object-cover border border-white/[0.05] shadow-xl"
                      />
                    ) : (
                      <img
                        src={URL.createObjectURL(selectedFile)}
                    ...
                    <button
                      ...
                      className="absolute -top-3 -right-3 w-8 h-8 bg-surface rounded-full text-brand flex items-center justify-center shadow-lg font-black border border-white/[0.05] z-10"
                    >
            ...
            {posts?.map((post: any) => {
              ...
              if (!isMember) {
                return (
                  <div
                    key={post.id}
                    className="flex flex-col bg-surface-card border border-white/[0.05] overflow-hidden shadow-2xl w-full relative rounded-[32px]"
                  >
                    <div className="flex items-center justify-between p-4 px-5 z-10 absolute top-0 left-0 right-0 bg-gradient-to-b from-surface/80 to-transparent rounded-t-[32px]">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col text-right">
                          <span className="text-brand font-black text-[14px] drop-shadow-sm">
                      ...
                      <div className="relative z-20 w-[280px] bg-surface/50 backdrop-blur-2xl rounded-[32px] p-6 flex flex-col items-center border border-white/[0.05] shadow-2xl">
                        <div className="absolute -top-7 relative flex justify-center w-full">
                          <div className="relative">
                            <img
                              src={post.profiles?.avatar_url || 'https://placehold.co/100x100/1E1F22/333'}
                              className="w-14 h-14 rounded-full border-4 border-surface-card object-cover"
                            />
                        ...
                        <h3 className="text-brand font-black mt-4 text-lg">
                          {levelTooLow ? 'נעול ע"י סלקטור' : 'Unlock to view'}
                        </h3>
                        ...
                        <button
                          ...
                          className={`mt-6 w-full font-black rounded-full py-3.5 text-sm uppercase tracking-widest shadow-lg transition-transform active:scale-95 flex items-center justify-center gap-2 ${
                            levelTooLow
                              ? 'bg-white/[0.03] text-brand-muted cursor-not-allowed shadow-none'
                        ...
                        >
                          {joining ? (
                            ...
                            `דרושה רמה ${requiredLevel}`
                          ) : circle.is_private && circle.join_price > 0 ? (
                            `הצטרף - ${circle.join_price} CRD`
                          ) : (
                            'הצטרף עכשיו'
                          )}
                        </button>
              ...
              return (
                <div
                  key={post.id}
                  className="flex flex-col bg-surface-card border border-white/[0.05] overflow-hidden shadow-xl rounded-[32px] w-full relative"
                >
                  <div className="flex items-center justify-between p-4 px-5 z-10 absolute top-0 left-0 right-0 bg-gradient-to-b from-surface/80 to-transparent rounded-t-[32px]">
                    <div
                    ...
                    <button
                      ...
                      className="text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] hover:text-white/70 transition-colors p-2"
                    >
                  ...
                  {hasMedia && (
                    <div
                      className="w-full bg-surface relative cursor-pointer rounded-t-[32px] overflow-hidden"
                      onClick={() =>
                        openOverlay(() => {
                          const vids = posts.filter((p: any) => p.media_url);
                          setFullScreenMedia([
                            post,
                            ...vids.filter((v: any) => v.id !== post.id).sort(() => Math.random() - 0.5),
                          ]);
                    ...
                    >
                      {isVideo ? (
                        ...
                        <img
                          src={post.media_url}
                          onError={(e) => {
                            e.currentTarget.src = 'https://placehold.co/500x500/1E1F22/333?text=Media+Unavailable';
                          }}
                          className="w-full aspect-[3/4] object-cover"
                        />
                      )}
                  ...
                  {!hasMedia && post.content && (
                    <div className="p-6 pb-4 mt-16">
                      <p
                        ...
                        className="text-white text-[15px] leading-relaxed text-right line-clamp-4 cursor-pointer active:opacity-50"
                      >
                        {post.content}
                      </p>
                    </div>
                  ...
                  <div className="flex items-center gap-6 px-5 py-4 bg-surface-card border-t border-white/[0.03]">
                    <button
                      ...
                      className={`flex items-center gap-2 transition-all active:scale-90 ${
                        post.is_liked ? 'text-red-500' : 'text-brand-muted hover:text-red-400'
                      }`}
                    >
                      <Heart
                        size={22}
                      ...
                      />
                      <span className="text-[14px] font-black text-white">{post.likes_count}</span>
                    </button>
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 text-brand-muted hover:text-accent-primary transition-all active:scale-90"
                    >
                      <MessageSquare size={22} />
                      <span className="text-[14px] font-black text-white">{post.comments_count}</span>
                    </button>

                    <button
                      onClick={() => handleShare(post)}
                      className="flex items-center gap-2 text-brand-muted hover:text-brand mr-auto active:scale-90 transition-all"
                    >
                      <Share2 size={20} />
                    </button>
                  </div>
          ...
          <AnimatePresence>
            {fullScreenMedia && (
              <motion.div
                ...
                exit={{ opacity: 0, y: 50 }}
                className="fixed inset-0 z-[999999] bg-[#000]"
              >
                    ...
                    return (
                      <div key={keyVal} className="w-full h-screen snap-center relative bg-[#000] flex items-center justify-center">
                        ...
                        <button
                          ...
                          className="absolute bottom-6 left-4 z-[60] active:scale-90 transition-transform drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]"
                        >
                          <MoreHorizontal size={26} strokeWidth={2.5} className="text-white" />
                        </button>
                        ...
                        <div className="absolute bottom-48 left-4 flex flex-col gap-6 items-center z-50">
                          ...
                          >
                            <Heart
                              size={30}
                              className={vid.is_liked ? 'text-[#ff4757]' : 'text-white'}
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-surface/90 via-surface/40 to-transparent flex flex-col justify-end pointer-events-none">
                          ...
                          >
                            <div className="w-12 h-12 rounded-full overflow-hidden bg-surface-card border-2 border-white/[0.05] shrink-0 shadow-lg">
                              {vid.profiles?.avatar_url ? (
                                ...
                                <UserCircle size={24} className="text-brand-muted w-full h-full p-2" />
                              )}
                            </div>
                          ...
                          <p
                            className="text-white text-[15px] font-medium text-right pr-2 w-5/6 line-clamp-3 pointer-events-auto cursor-pointer"
                            onClick={(e) => {
                              e.stopPropagation();
            ...
            {activeCommentsPostId && (
              <div className="fixed inset-0 z-[99999] flex flex-col justify-end" dir="rtl" onTouchStart={stopPropagation} onTouchMove={stopPropagation}>
                ...
                <motion.div
                  ...
                  className="relative z-10 bg-surface-card rounded-t-[40px] h-[85vh] flex flex-col overflow-hidden pb-10 shadow-[0_-20px_50px_rgba(0,0,0,0.8)] border-t border-white/[0.05]"
                >
                  <div
                          ...
                          return (
                            <div key={c.id} className="flex flex-col gap-2">
                              <div className="flex gap-3">
                                <div
                                  className="w-10 h-10 min-w-[40px] rounded-full bg-surface shrink-0 overflow-hidden cursor-pointer border border-white/[0.05] flex items-center justify-center"
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    closeOverlay();
                                ...
                                <div className="flex flex-col flex-1">
                                  <div
                                    className="bg-surface p-3 rounded-[24px] rounded-tr-sm cursor-pointer"
                              ...
                              {replies.length > 0 && (
                                <div className="pr-10 flex flex-col gap-2">
                                  {replies.map((reply) => (
                                    <div key={reply.id} className="bg-surface rounded-[20px] p-3 border border-white/[0.05]">
                                      <div className="text-brand text-xs font-bold mb-1">{reply.profiles?.full_name || 'אנונימי'}</div>
                                      <div className="text-brand-muted text-sm">{reply.content}</div>
                                    </div>
                                  ))}
                                </div>
                              )}
                  ...
                  <div className="p-4 bg-surface border-t border-white/[0.05] flex gap-2 pb-8">
                    <input
                      ...
                      className="flex-1 bg-surface-card border border-white/[0.05] text-brand rounded-full px-5 outline-none text-[15px]"
                    />
                    <Button
                      onClick={submitComment}
                      disabled={!newComment.trim()}
                      className="w-14 h-14 p-0 rounded-full shrink-0 bg-accent-primary text-surface shadow-md hover:bg-accent-primary/90"
                    >
            ...
            {optionsMenuPost && (
              <div className="fixed inset-0 z-[99999] flex flex-col justify-end" onTouchStart={stopPropagation} onTouchMove={stopPropagation}>
                <motion.div
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="absolute inset-0 z-0 bg-surface/80 backdrop-blur-sm"
                  onClick={closeOverlay}
                />
                ...
                <motion.div
                  ...
                  animate={{ y: 0 }}
                  exit={{ y: '100%' }}
                  className="relative z-10 bg-surface-card rounded-t-[40px] p-6 flex flex-col gap-2 pb-12 shadow-[0_-10px_50px_rgba(0,0,0,0.8)] border-t border-white/[0.05]"
                >
                  ...
                  <button
                    ...
                    className="w-full p-4 bg-surface rounded-full text-brand font-bold flex justify-between items-center text-lg active:bg-surface/80 transition-colors border border-white/[0.05]"
                  >
                    <span>שתף פוסט</span>
                    <Share2 size={20} className="text-brand-muted" />
                  </button>

                  {optionsMenuPost.media_url && (
                    <button
                      onClick={() => handleDownloadMedia(optionsMenuPost.media_url)}
                      className="w-full p-4 bg-surface rounded-full text-brand font-bold flex justify-between items-center text-lg active:bg-surface/80 transition-colors mt-2 border border-white/[0.05]"
                    >
                      <span>שמור למכשיר</span>
                      <Download size={20} className="text-brand-muted" />
                    </button>
                  )}
                  ...
                  >
                    ...
                    <Bookmark size={20} className="text-brand-muted" />
                  </button>
                  ...
                  >
                    ...
                    <Link size={20} className="text-brand-muted" />
                  </button>
                  ...
                  {optionsMenuPost.user_id === currentUserId && (
                    <>
                      <button
                        ...
                        className="w-full p-4 bg-surface rounded-full text-brand font-bold flex justify-between items-center text-lg active:bg-surface/80 transition-colors mt-4 border border-white/[0.05]"
                      >
                        <span>ערוך פוסט</span>
                        <Edit2 size={20} className="text-brand-muted" />
                      </button>
            ...
            {commentActionModal && (
              <div className="fixed inset-0 z-[100000] flex flex-col justify-end" onTouchStart={stopPropagation} onTouchMove={stopPropagation}>
                ...
                <motion.div
                  ...
                  animate={{ y: 0 }}
                  exit={{ y: '100%' }}
                  className="relative z-10 bg-surface-card rounded-t-[40px] p-6 flex flex-col gap-3 pb-12 shadow-[0_-10px_50px_rgba(0,0,0,0.8)] border-t border-white/[0.05]"
                >
                  <div className="w-full py-4 flex justify-center cursor-grab active:cursor-grabbing">
                    <div className="w-16 h-1.5 bg-white/[0.1] rounded-full" />
                  </div>
                  ...
                  <button
                    ...
                    className="w-full p-4 bg-surface rounded-full text-brand font-bold flex justify-between items-center text-lg hover:bg-surface/80 transition-colors border border-white/[0.05]"
                  >
                    <span>השב לתגובה</span>
                    <Reply size={20} className="text-brand-muted" />
                  </button>
            ...
            {activeDescPost && (
              <div className="fixed inset-0 z-[99999] flex flex-col justify-end" onTouchStart={stopPropagation} onTouchMove={stopPropagation}>
                ...
                <motion.div
                  ...
                  animate={{ y: 0 }}
                  exit={{ y: '100%' }}
                  className="relative z-10 bg-surface-card rounded-t-[40px] flex flex-col overflow-hidden pb-10 max-h-[75vh] shadow-[0_-10px_50px_rgba(0,0,0,0.8)] border-t border-white/[0.05]"
                >
                  <div className="w-full py-6 flex justify-center cursor-grab active:cursor-grabbing border-b border-white/[0.05]">
                    <div className="w-16 h-1.5 bg-white/[0.1] rounded-full" />
                  </div>
                  <div className="px-6 py-4 border-b border-white/[0.05]">
                    <h2 className="text-brand font-black text-lg text-center">תיאור מלא</h2>
                  </div>
                  <div className="p-6 overflow-y-auto" onPointerDown={stopPropagation} onTouchStart={stopPropagation}>
                    <p className="text-brand-muted text-[15px] leading-relaxed text-right whitespace-pre-wrap">
