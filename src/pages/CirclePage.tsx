  ...
  const handlePost = async () => {
    ...
    try {
      ...
      if (selectedFile) {
        ...
        const fileName = `chat_${Date.now()}_${Math.random().toString(36).slice(2, 6)}_${safeName}`;
          ...
          <div className="flex flex-col flex-1 overflow-hidden relative z-10 mt-6">
            ...
            {activeTab === 'chat' && (
              <div className="flex-1 relative flex flex-col overflow-hidden">
                          ...
                          <div className={`relative flex flex-col max-w-[85%] rounded-[24px] ${post.user_id === currentUserId ? 'rounded-br-sm' : 'rounded-bl-sm'} overflow-hidden shadow-sm ${!post.media_url ? (post.user_id === currentUserId ? 'bg-accent-primary/20 border border-accent-primary/30 text-white' : 'bg-white/5 border border-white/5 text-white/90') : ''}`}>
                            ...
                            {post.media_url ? (
                              <div 
                                ...
                                onClick={() => setFullScreenMedia({url: post.media_url, type: post.media_type})}
                              >
                                {post.media_type === 'video' ? (
                                  <video src={post.media_url} autoPlay muted loop playsInline className="w-full h-auto max-h-[350px] object-cover" />
                                ) : (
                              ...
                              <div className="px-4 py-3">
                                <span className="text-[14px] leading-relaxed font-medium whitespace-pre-wrap break-words">{post.content}</span>
                              </div>
                            )}
                          </div>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-[#050505] via-[#050505]/90 to-transparent z-[100] pb-[calc(env(safe-area-inset-bottom)+16px)]">
