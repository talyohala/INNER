...
export const CirclePage: React.FC = () => {
  ...
  // Tabs (Chat -> לייב צ'אט, Vaults -> כספות, Members -> חברים)
  ...
  // Fetch Vaults when tab changes to vaults
  useEffect(() => {
    if (activeTab === 'vaults' && data?.circle?.slug) {
      fetchVaults();
    }
  }, [activeTab, data?.circle?.slug]);
  ...
  const fetchCircleData = async () => {
    try {
      ...
      const { data: pData } = await supabase.from('posts')
        ...
        .is('parent_id', null) // Only main posts for feed
  ...
  const loadMorePosts = async () => {
    ...
    try {
      ...
      const { data: pData } = await supabase.from('posts').select('*, profiles!user_id(*), seals:post_seals(id, seal_type, user_id), comments(id)')
        ...
        .is('parent_id', null)
        .order('created_at', { ascending: false }).range(nextPage * POSTS_PER_PAGE, (nextPage + 1) * POSTS_PER_PAGE - 1);
  ...
  const handlePost = async (isGift = false, giftAmount = 0) => {
    ...
    try {
      if (isGift) {
        if ((myProfile?.crd_balance || 0) < giftAmount) { toast.error('אין לך מספיק CRD. קפוץ לארנק.'); setPosting(false); return; }
      ...
      if (replyingToPost && !isGift) {
        ...
        setReplyingToPost(null); setNewPost(''); setPosting(false); return;
      }
      ...
      if (editingPost) {
        ...
        if (selectedFile) {
          ...
          if (uploadData) {
            ...
            media_types = [selectedFile.type.startsWith('video/') ? 'video' : 'image'];
          }
        }
  ...
  const handleSeal = async (postId: string, sealType: string) => {
    ...
    triggerFeedback('pop'); closeOverlay();
  ...
  const handleContainerScroll = (e: React.UIEvent<HTMLDivElement>) => {
    ...
    scrollTimeout.current = setTimeout(() => {
      ...
      if (target.scrollHeight - target.scrollTop <= target.clientHeight * 2 && fullScreenMedia) {
        const mediaPosts = data.posts.filter((p: any) => p.media_urls?.length > 0);
      ...
      <FadeIn className="bg-surface h-[100dvh] font-sans flex flex-col relative overflow-hidden" dir="rtl">
        ...
        {!isMember ? (
            ...
            {!levelTooLow && (
              <div className="w-full flex flex-col gap-3 mt-4 pb-10">
                <Button onClick={() => handleJoin('INNER')} disabled={joining} className="w-full h-14 bg-surface-card border border-surface-border text-brand font-black rounded-[20px] shadow-sm active:scale-95 transition-all uppercase tracking-widest text-[13px] hover:bg-white/5">
                  {joining ? <Loader2 size={18} className="animate-spin" /> : `כניסה רגילה - ${circle.join_price || 0} CRD`}
                </Button>
                <Button onClick={() => handleJoin('CORE')} disabled={joining} className="w-full h-14 bg-white text-black font-black rounded-[20px] shadow-[0_5px_20px_rgba(255,255,255,0.15)] flex items-center justify-center gap-2 active:scale-95 transition-all uppercase tracking-widest text-[13px]">
                  {joining ? <Loader2 size={18} className="animate-spin text-black" /> : <>שדרוג ל-CORE - {circle.core_price || (circle.join_price * 2) || 50} CRD</>}
                </Button>
                <span className="text-[10px] font-bold text-brand-muted/70 tracking-widest uppercase text-center w-full block mt-2">כולל גישה לכספות ופוסטים מודגשים</span>
              </div>
            )}
          </div>
        ) : (
          ...
          <div className="flex flex-col flex-1 overflow-hidden relative">
                            ...
                            <div className={`rounded-[24px] shadow-sm flex flex-col overflow-hidden ${post.gift_amount ? 'bg-surface-card border border-accent-primary/40 shadow-[0_0_15px_rgba(var(--color-accent-primary),0.1)]' : isMine ? 'bg-surface-card border border-surface-border rounded-tr-sm' : 'bg-surface border border-surface-border rounded-tl-sm'}`}>
                              ...
                              {hasMedia && firstMedia && (
                                <div className="w-full relative bg-black cursor-pointer" onClick={() => { if (!isVideo) openOverlay(() => { setFullScreenMedia([post]); setCurrentMediaIndex(0); }); }}>
                                  {isVideo ? (
                                    <FeedVideo src={firstMedia} className={`w-full max-h-[300px] object-cover`} />
                                  ) : (
                                    <img src={firstMedia} loading="lazy" className={`w-full max-h-[300px] object-cover`} />
                                  )}
                                </div>
                              )}
                              
                              {post.content && (
                                <p className={`text-[14px] leading-relaxed whitespace-pre-wrap px-4 ${hasMedia ? 'py-3' : 'py-3.5'} ${post.gift_amount ? 'text-brand font-medium' : 'text-brand'}`}>
                                  {post.content}
                                </p>
                              )}
                            </div>
            ...
            {/* 📦 VAULTS TAB */}
            {activeTab === 'vaults' && (
              <div className="flex-1 p-4 flex flex-col gap-6 bg-surface overflow-y-auto pb-[120px]">
                {loadingVaults ? (
                  <div className="flex flex-col items-center justify-center py-20 gap-3">
                    <Loader2 className="animate-spin text-accent-primary" size={32} />
                    <span className="text-[10px] font-black text-brand-muted uppercase tracking-widest">טוען כספות...</span>
                  </div>
                ) : vaults.length === 0 ? (
                  <div className="flex flex-col items-center text-center py-20 opacity-50">
                    <Lock size={48} className="text-brand-muted mb-4" />
                    <h3 className="text-lg font-black text-brand mb-1">אין כספות זמינות</h3>
                    <p className="text-xs font-bold text-brand-muted max-w-[200px]">עדיין לא עלה תוכן נעול למועדון הזה.</p>
                  </div>
                ) : (
                  vaults.map(vault => (
              ...
              <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }} className="fixed inset-0 z-[999999] bg-surface">
                <div className="w-full h-full overflow-y-scroll snap-y snap-mandatory scrollbar-hide" onScroll={handleContainerScroll}>
                  {fullScreenMedia.map((vid, idx) => {
                    const isVid = vid.media_urls?.[0]?.match(/\.(mp4|webm|mov)$/i);
                    ...
                    return (
                      <div key={keyVal} className="w-full h-screen snap-center relative bg-surface flex items-center justify-center">
                        {isVid ? <video src={vid.media_urls[0]} loop playsInline className="w-full h-full object-cover" onClick={(e) => (e.currentTarget.paused ? e.currentTarget.play() : e.currentTarget.pause())} /> : <img src={vid.media_urls?.[0]} className="w-full h-full object-contain" />}
                        <button onClick={(e) => { e.stopPropagation(); closeOverlay(); }} className="absolute top-6 left-4 z-[60] active:scale-90 transition-transform bg-surface-card/60 backdrop-blur-md border border-white/10 rounded-full p-3"><X size={20} className="text-white" /></button>
                      </div>
                    );
                  })}
                </div>
